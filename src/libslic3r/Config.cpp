...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    {
        //BBS
        const std::string legacy_orcaslicer_gcode_header = "; OrcaSlicer";
        const std::string himstudio_gcode_header         = std::string("; generated by ") + SLIC3R_APP_NAME;
        const std::string orcaslicer_gcode_header        = "; generated by OrcaSlicer";
        ...
        while (std::getline(ifs, header)) {
            ...
            // BBS
            if (strncmp(legacy_orcaslicer_gcode_header.c_str(), line_c, strlen(legacy_orcaslicer_gcode_header.c_str())) == 0 ||
                strncmp(himstudio_gcode_header.c_str(), line_c, strlen(himstudio_gcode_header.c_str())) == 0 ||
