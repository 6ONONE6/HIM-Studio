...
void PresetBundle::setup_directories()
{
    ...
    std::initializer_list<boost::filesystem::path> paths = {
        data_dir, data_dir / "ota", data_dir / PRESET_SYSTEM_DIR, data_dir / PRESET_USER_DIR,
...
void PresetBundle::update_selections(AppConfig& config)
{
    ...
    filament_colors.resize(filament_presets.size(), "#2573D9");
...
void PresetBundle::load_selections(AppConfig& config, const PresetPreferences& preferred_selection /* = PresetPreferences()*/)
{
    ...
    filament_colors.resize(filament_presets.size(), "#2573D9"); // 26A69A
    project_config.option<ConfigOptionStrings>("filament_colour")->values = filament_colors;
    std::vector<std::string> matrix;
    if (config.has_printer_setting(initial_printer_profile_name, "flush_volumes_matrix")) {
        boost::algorithm::split(matrix, config.get_printer_setting(initial_printer_profile_name, "flush_volumes_matrix"),
...
{
    return (this->printers.get_edited_preset().printer_technology() == ptFFF) ? this->full_fff_config() : this->full_sla_config();
}
...
void PresetBundle::load_config_file_config(const std::string&   name_or_path,
                                           bool                 is_external,
                                           DynamicPrintConfig&& config,
                                           Semver               file_version,
                                           bool                 selected,
                                           bool                 is_custom_defined)
{
    ...
    auto load_preset = [&config, &inherits, &inherits_values, &compatible_printers_condition, &compatible_printers_condition_values,
                        ...
                        selected, is_custom_defined](PresetCollection& presets, size_t idx, const std::string& key,
        ...
        if (is_external)
            ...
            presets.load_preset(presets.path_from_name(name, inherits.empty()), name, config, selected, file_version, is_custom_defined)
                .save(nullptr);
    };
        ...
        if (num_filaments <= 1) {
            ...
            if (is_external)
                ...
                loaded = &this->filaments.load_preset(this->filaments.path_from_name(name, inherits.empty()), name, config, true,
                                                      file_version, is_custom_defined);
                loaded->save(nullptr);
            }
            this->filament_presets.clear();
