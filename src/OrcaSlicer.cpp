...
std::map<int, std::string> cli_errors =
    {{CLI_SUCCESS, "Success."},
     ...
     {CLI_FILE_VERSION_NOT_SUPPORTED,
...
typedef struct _cli_callback_mgr
{
    ...
    void notify()
    {
        ...
        // notify_message = "Plate "+ std::to_string(m_plate_index) + "/" +std::to_string(m_plate_count)+  ": Percent " +
        // std::to_string(m_progress) + ": "+m_message;
    ...
    void update(int percent, std::string message, int warning_step)
    {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": percent=" << percent << ", warning_step=" << warning_step
                                << ", plate_index = " << m_plate_index << ", plate_count=" << m_plate_count << ", message=" << message;
        if (warning_step == -1) {
            m_progress = percent;
    ...
    bool start(std::string pipe_name)
    {
        ...
        while (m_pipe_fd < 0) {
            if ((retry_count % 10) == 0)
                BOOST_LOG_TRIVIAL(warning) << boost::format("could not open pipe for %1%, errno %2%, reason: %3%, retry_count = %4%") %
                                                  pipe_name % errno % strerror(errno) % retry_count;
            retry_count++;
            if (retry_count >= 50) {
        ...
        std::unique_lock<std::mutex> lck(m_mutex);
        m_thread = create_thread([this] { this->thread_proc(); });
...
void               cli_status_callback(const PrintBase::SlicingStatus& slicing_status)
{
    if (slicing_status.warning_step != -1) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": percent=%1%, warning_step=%2%, message=%3%, message_type=%4%, flag=%5%") %
                                       slicing_status.percent % slicing_status.warning_step % slicing_status.text %
                                       (int) (slicing_status.message_type) % slicing_status.flags;
    }
...
void default_status_callback(const PrintBase::SlicingStatus& slicing_status)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                             << boost::format(": percent=%1%, warning_step=%2%, message=%3%, message_type=%4%") % slicing_status.percent %
                                    slicing_status.warning_step % slicing_status.text % (int) (slicing_status.message_type);
...
#if defined(__linux__) || defined(__LINUX__)
#define flush_and_exit(ret) \
    { \
        boost::nowide::cout << __FUNCTION__ << " found error, return " << ret << ", exit..." << std::endl; \
...
#define flush_and_exit(ret) \
    { \
        boost::nowide::cout << __FUNCTION__ << " found error, exit" << std::endl; \
        boost::nowide::cout.flush(); \
        boost::nowide::cerr.flush(); \
        for (Model & model : m_models) { \
            model.remove_backup_path_if_exist(); \
...
void record_exit_reson(std::string                        outputdir,
                       int                                code,
                       int                                plate_id,
    ...
    try {
        ...
        for (int plate_index = 0; plate_index < plate_count; plate_index++) {
            ...
            if (plate_json.contains(JSON_ASSEMPLE_PLATE_PARAMS)) {
                ...
                BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, has %2% plate params") % (plate_index + 1) %
                                                assemble_plate.plate_params.size();
            }

            int object_count = plate_json[JSON_ASSEMPLE_OBJECTS].size();
            if (object_count <= 0) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(": invalid object count %1% in plate %2%") % object_count % (plate_index + 1);
                return CLI_CONFIG_FILE_ERROR;
            }
            ...
            for (int object_index = 0; object_index < object_count; object_index++) {
                ...
                if (assemble_object.count <= 0) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                             << boost::format(": invalid object clone count %1% in plate %2% Object %3%") %
                                                    assemble_object.count % (plate_index + 1) % assemble_object.path;
                    return CLI_CONFIG_FILE_ERROR;
                }

                assemble_object.filaments = object_json.at(JSON_ASSEMPLE_OBJECT_FILAMENTS).get<std::vector<int>>();
                if ((assemble_object.filaments.size() > 0) && (assemble_object.filaments.size() != assemble_object.count) &&
                    (assemble_object.filaments.size() != 1)) {
                    BOOST_LOG_TRIVIAL(error)
                        << __FUNCTION__
                        << boost::format(": object %1%'s filaments count %2% not equal to clone count %3%, also not equal to 1") %
                               assemble_object.path % assemble_object.filaments.size() % assemble_object.count;
                    return CLI_CONFIG_FILE_ERROR;
                }
                ...
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_ASSEMBLE_INDEX)) {
                    ...
                    if ((assemble_object.assemble_index.size() > 0) && (assemble_object.assemble_index.size() != assemble_object.count) &&
                        (assemble_object.assemble_index.size() != 1)) {
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__
                            << boost::format(": object %1%'s assemble_index count %2% not equal to clone count %3%, also not equal to 1") %
                                   assemble_object.path % assemble_object.assemble_index.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                ...
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_POS_X)) {
                    ...
                    if ((assemble_object.pos_x.size() > 0) && (assemble_object.pos_x.size() != assemble_object.count) &&
                        (assemble_object.pos_x.size() != 1)) {
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__
                            << boost::format(": object %1%'s pos_x count %2% not equal to clone count %3%, also not equal to 1") %
                                   assemble_object.path % assemble_object.pos_x.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_POS_Y)) {
                    ...
                    if ((assemble_object.pos_y.size() > 0) && (assemble_object.pos_y.size() != assemble_object.count) &&
                        (assemble_object.pos_y.size() != 1)) {
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__
                            << boost::format(": object %1%'s pos_y count %2% not equal to clone count %3%, also not equal to 1") %
                                   assemble_object.path % assemble_object.pos_y.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_POS_Z)) {
                    ...
                    if ((assemble_object.pos_z.size() > 0) && (assemble_object.pos_z.size() != assemble_object.count) &&
                        (assemble_object.pos_z.size() != 1)) {
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__
                            << boost::format(": object %1%'s pos_z count %2% not equal to clone count %3%, also not equal to 1") %
                                   assemble_object.path % assemble_object.pos_z.size() % assemble_object.count;
                        return CLI_CONFIG_FILE_ERROR;
                    }
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_PRINT_PARAMS)) {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, object %2% has %3% print params") % (plate_index + 1) %
                                                    assemble_object.path % assemble_object.print_params.size();
                }
                if (object_json.contains(JSON_ASSEMPLE_OBJECT_HEIGHT_RANGES)) {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, object %2% has %3% height ranges") % (plate_index + 1) %
                                                    assemble_object.path % range_count;
                    ...
                    for (int range_index = 0; range_index < range_count; range_index++) {
                        ...
                        height_range.range_params         = height_range_json[range_index][JSON_ASSEMPLE_OBJECT_RANGE_PARAMS]
            ...
            if (plate_json.contains(JSON_ASSEMPLE_ASSEMBLE_PARAMS)) {
                ...
                for (int i = 0; i < assemble_count; i++) {
                    ...
                    if (assemble_params_json[i].contains(JSON_ASSEMPLE_OBJECT_PRINT_PARAMS)) {
                        assembled_param.print_params = assemble_params_json[i][JSON_ASSEMPLE_OBJECT_PRINT_PARAMS]
                                                           .get<std::map<std::string, std::string>>();
                        BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, assemble object %2% has %3% print params") %
                                                        (plate_index + 1) % i % assembled_param.print_params.size();
                    }
                    if (assemble_params_json[i].contains(JSON_ASSEMPLE_OBJECT_HEIGHT_RANGES)) {
                        ...
                        BOOST_LOG_TRIVIAL(debug)
                            << boost::format("Plate %1%, assemble object %2% has %3% height ranges") % (plate_index + 1) % i % range_count;
...
void merge_or_add_object(assemble_plate_info_t&       assemble_plate_info,
                         Model&                       model,
                         int                          assemble_index,
                         std::map<int, ModelObject*>& merged_objects,
                         ModelObject*                 ori_object)
{
    if (assemble_index > 0) {
        ...
        BOOST_LOG_TRIVIAL(debug) << boost::format("assemble_index %1%, name %2%, merged to new model %3%") % assemble_index %
                                        ori_object->name % new_object->name;
    } else {
        ModelObject* new_object = model.add_object(*ori_object);
        assemble_plate_info.loaded_obj_list.emplace_back(new_object);
        BOOST_LOG_TRIVIAL(debug) << boost::format("assemble_index %1%, name %2%, no need to merge, copy to new model") % assemble_index %
                                        ori_object->name;
    }
}
...
static int construct_assemble_list(std::vector<assemble_plate_info_t>& assemble_plate_info_list, Model& model, PlateDataPtrs& plate_list)
{
    ...
    for (int index = 0; index < plate_count; index++) {
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": Plate %1%, name %2%, obj count %3%, plate params count %4%") % (index + 1) %
                                       assemble_plate_info.plate_name % object_count % assemble_plate_info.plate_params.size();
        ...
        if (!assemble_plate_info.plate_params.empty()) {
            for (auto plate_iter = assemble_plate_info.plate_params.begin(); plate_iter != assemble_plate_info.plate_params.end();
                 plate_iter++) {
                plate_data->config.set_deserialize(plate_iter->first, plate_iter->second, config_substitutions);
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(": Plate %1%, key %2%, value %3%") % (index + 1) % plate_iter->first %
                                                plate_iter->second;
            }
        }
        ...
        for (size_t obj_index = 0; obj_index < object_count; obj_index++) {
            ...
            if (!fs::exists(object_path)) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(": directory %1% not exist in plate %2%") % assemble_object.path % (index + 1);
                return CLI_FILE_NOTFOUND;
            }
            ...
            if (boost::algorithm::iends_with(assemble_object.path, ".stl")) {
                if (!mesh.ReadSTLFile(path_str, true, nullptr)) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                             << boost::format(": failed to read stl file from %1%, plate index %2%, object index %3%") %
                                                    assemble_object.path % (index + 1) % (obj_index + 1);
                    return CLI_DATA_FILE_ERROR;
                }
                if (mesh.empty()) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                             << boost::format(": found no mesh data from stl file %1%, plate index %2%, object index %3%") %
                                                    assemble_object.path % (index + 1) % (obj_index + 1);
                    return CLI_DATA_FILE_ERROR;
                }
                ...
                if (!result) {
                    BOOST_LOG_TRIVIAL(error)
                        << __FUNCTION__
                        << boost::format(": failed to read a valid mesh from obj file %1%, plate index %2%, object index %3%, error %4%") %
                               assemble_object.path % (index + 1) % (obj_index + 1) % message;
                    return CLI_DATA_FILE_ERROR;
                }
                object_name.erase(object_name.end() - 3, object_name.end());
            } else {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(": unsupported file %1%, plate index %2%, object index %3%") %
                                                assemble_object.path % (index + 1) % (obj_index + 1);
            ...
            if (!object) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(": add_object %1% failed, plate index %2%, object index %3%") % object_1_name %
                                                (index + 1) % (obj_index + 1);
                return CLI_DATA_FILE_ERROR;
            }
            ...
            if (!assemble_object.print_params.empty()) {
                for (auto param_iter = assemble_object.print_params.begin(); param_iter != assemble_object.print_params.end();
                     param_iter++) {
                    object->config.set_deserialize(param_iter->first, param_iter->second, config_substitutions);
                    BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, object %2% key %3%, value %4%") % (index + 1) % object_1_name %
                                                    param_iter->first % param_iter->second;
                }
            }
            ...
            if (!assemble_object.height_ranges.empty()) {
                for (int range_index = 0; range_index < assemble_object.height_ranges.size(); range_index++) {
                    ...
                    for (auto range_config_iter = range.range_params.begin(); range_config_iter != range.range_params.end();
                         range_config_iter++) {
                        range_config.set_deserialize(range_config_iter->first, range_config_iter->second, config_substitutions);
                        BOOST_LOG_TRIVIAL(debug) << boost::format("object %1%, height range %2% key %3%, value %4%") % object_1_name %
                                                        range_index % range_config_iter->first % range_config_iter->second;
                    }
            ...
            BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                     << boost::format(
                                            ": object %1%, name %2%, pos_x %3% pos_y %4%, pos_z %5%, filament %6%, assemble_index %7%") %
                                            obj_index % object->name % assemble_object.pos_x[0] % assemble_object.pos_y[0] %
                                            assemble_object.pos_z[0] % assemble_object.filaments[0] % assemble_object.assemble_index[0];
            ...
            for (size_t copy_index = 1; copy_index < assemble_object.count; copy_index++) {
                ...
                if (copy_index >= assemble_object.pos_x.size())
                    array_index = 0;
                copy_obj->translate(assemble_object.pos_x[array_index], assemble_object.pos_y[array_index],
                                    assemble_object.pos_z[array_index]);
                ...
                BOOST_LOG_TRIVIAL(debug) << __FUNCTION__
                                         << boost::format(": cloned object %1%, name %2%, pos_x %3% pos_y %4%, pos_z %5%") % copy_index %
                                                object->name % assemble_object.pos_x[array_index] % assemble_object.pos_y[array_index] %
                                                assemble_object.pos_z[array_index];
            }
        }
            ...
            for (auto& iter : merged_objects) {
                ...
                if (assemble_iter != assemble_plate_info.assembled_param_list.end()) {
                    ...
                    if (!assembled_param.print_params.empty()) {
                        for (auto param_iter = assembled_param.print_params.begin(); param_iter != assembled_param.print_params.end();
                             param_iter++) {
                            assemble_obj->config.set_deserialize(param_iter->first, param_iter->second, config_substitutions);
                            BOOST_LOG_TRIVIAL(debug) << boost::format("Plate %1%, assemble object %2% key %3%, value %4%") % (index + 1) %
                                                            assemble_obj->name % param_iter->first % param_iter->second;
                        }
                    }
                    ...
                    if (!assembled_param.height_ranges.empty()) {
                        for (int range_index = 0; range_index < assembled_param.height_ranges.size(); range_index++) {
                            ...
                            for (auto range_config_iter = range.range_params.begin(); range_config_iter != range.range_params.end();
                                 range_config_iter++) {
                                range_config.set_deserialize(range_config_iter->first, range_config_iter->second, config_substitutions);
                                BOOST_LOG_TRIVIAL(debug) << boost::format("assenble object %1%, height range %2% key %3%, value %4%") %
                                                                assemble_obj->name % range_index % range_config_iter->first %
                                                                range_config_iter->second;
                            }
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                << boost::format(": Plate %1%, used filaments %2%") % (index + 1) % assemble_plate_info.filaments_count;
    }

    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                            << boost::format(": has objects need to be merged, total plates %1%, total objects %2%") % plate_count %
                                   model.objects.size();
...
static void load_downward_settings_list_from_config(std::string               config_file,
                                                    std::string               printer_name,
                                                    std::string               printer_model,
                                                    std::vector<std::string>& downward_settings)
{
    std::map<std::string, std::string> printer_params;

    boost::filesystem::path directory_path(config_file);

    BOOST_LOG_TRIVIAL(info) << boost::format("%1%, will parse file %2% for printer mode %3%, printer name %4%") % __FUNCTION__ %
                                   config_file % printer_model % printer_name;
    if (!fs::exists(directory_path)) {
        ...
        try {
                ...
                if (!printer_model.empty() && printer_json.contains(printer_model)) {
                    ...
                    if (printer_model_json.contains("downward_check")) {
                        ...
                        if (downward_check_json.contains(printer_name)) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << boost::format("got %1% downward settings of %2% in cli_config.json") %
                                                           downward_settings.size() % printer_name;
                        } else {
                            BOOST_LOG_TRIVIAL(info) << boost::format("can not find  %1% in downward_check of %2% in cli_config.json") %
            ...
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": parse file " << config_file
...
int CLI::run(int argc, char** argv)
{
    ...
    // m_config.option<ConfigOptionEnum<ForwardCompatibilitySubstitutionRule>>("config_compatibility", true)->value;
    ...
    bool first_file = true, is_bbl_3mf = false, need_arrange = true, has_thumbnails = false, up_config_to_date = false,
         normative_check = true, duplicate_single_object = false, use_first_fila_as_default = false, minimum_save = false,
    ...
    std::string            new_printer_name, current_printer_name, new_process_name, current_process_name, current_printer_system_name,
        current_process_system_name, new_process_system_name, new_printer_system_name, printer_model_id, current_printer_model,
        printer_model; //, printer_inherits, print_inherits;
    std::vector<std::string> upward_compatible_printers, new_print_compatible_printers, current_print_compatible_printers,
        current_different_settings;
    ...
    BOOST_LOG_TRIVIAL(info)
        << boost::format("allow_multicolor_oneplate %1%, allow_rotations %2% skip_modified_gcodes %3% avoid_extrusion_cali_region %4% "
                         "loaded_filament_ids size %5%, clone_objects size %6%, skip_useless_pick %7%, allow_newer_file %8%") %
               allow_multicolor_oneplate % allow_rotations % skip_modified_gcodes % avoid_extrusion_cali_region %
               loaded_filament_ids.size() % clone_objects.size() % skip_useless_pick % allow_newer_file;
    if (clone_objects.size() > 0) {
        if (clone_objects.size() != m_input_files.size()) {
            BOOST_LOG_TRIVIAL(error) << boost::format("clone_objects size %1% should be the same with input files size %2%") %
                                            clone_objects.size() % m_input_files.size();
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
    ...
    if (loaded_filament_ids.size() > 0) {
        if (loaded_filament_ids.size() != m_input_files.size()) {
            BOOST_LOG_TRIVIAL(error) << boost::format("loaded_filament_ids size %1% should be the same with input files size %2%") %
                                            loaded_filament_ids.size() % m_input_files.size();
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
        } else if (load_filaments.size() == 0) {
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("plate_to_slice=%1%, normative_check=%2%, use_first_fila_as_default=%3%") % plate_to_slice %
    ...
    if (!load_assemble_list.empty() && ((m_input_files.size() > 0) || (m_transforms.size() > 0))) {
        BOOST_LOG_TRIVIAL(error) << boost::format(
            "load_assemble_list should not be used with input model files to load and should not be sued with transforms");
        record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
        flush_and_exit(CLI_INVALID_PARAMS);
    }
        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (boost::algorithm::iends_with(file, ".3mf") && first_file) {
                    ...
                    strategy = LoadStrategy::LoadModel | LoadStrategy::LoadConfig | LoadStrategy::AddDefaultInstances |
                ...
                if (is_bbl_3mf) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("the first file is a 3mf, version %1%, got plate count %2%") %
                                                   file_version.to_string() % plate_data_src.size();
                    need_arrange = false;
                    ...
                    if (!allow_newer_file && ((cli_ver.maj() != file_version.maj()) || (cli_ver.min() < file_version.min()))) {
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("Version Check: File Version %1% not supported by current cli version %2%") %
                                   file_version.to_string() % SLIC3R_VERSION;
                        record_exit_reson(outfile_dir, CLI_FILE_VERSION_NOT_SUPPORTED, 0, cli_errors[CLI_FILE_VERSION_NOT_SUPPORTED],
                                          sliced_info);
                    ...
                    if ((file_version < old_version2) && !config.empty()) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("old 3mf version %1%, need to regenerate_thumbnails for all") % file_version.to_string();
                    }
                    ...
                    /*for (ModelObject *model_object : model.objects)
                        for (ModelInstance *model_instance : model_object->instances)
                        {
                            ...
                            BOOST_LOG_TRIVIAL(info) << boost::format("instance %1% transform {%2%,%3%,%4%} at %5%:%6%")% model_object->name
                       % instance_offset.x() % instance_offset.y() %instance_offset.z() % __FUNCTION__ % __LINE__<< std::endl;
                        }*/
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("current_printer_name %1%, current_process_name %2%") % current_printer_name %
                    ...
                    if (option_strings) {
                        ...
                        if (current_inherits_group[size - 1].empty()) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << boost::format("inherits of printer valid, current_printer_system_name is %1%") %
                                                           current_printer_system_name;
                        }
                        ...
                        if (current_inherits_group[0].empty()) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << boost::format("inherits of process valid, current_process_system_name is %1%") %
                                                           current_process_system_name;
                        }
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("old printable size from 3mf: {%1%, %2%, %3%}") % old_printable_width %
                                                   old_printable_depth % old_printable_height;
                    BOOST_LOG_TRIVIAL(info) << boost::format("old extruder_clearance_height_to_rod %1%, extruder_clearance_height_to_lid "
                                                             "%2%, extruder_clearance_radius %3%}") %
                                                   old_height_to_rod % old_height_to_lid % old_max_radius;
                } else {
                    need_arrange           = true;
                    ...
                    if (loaded_filament_ids.size() > input_index) {
                        if (loaded_filament_ids[input_index] > 0) {
                            if (loaded_filament_ids[input_index] > load_filaments.size()) {
                                BOOST_LOG_TRIVIAL(error) << boost::format("invalid filament_id %1% at index %2%, max %3%") %
                                                                loaded_filament_ids[input_index] % (input_index + 1) %
                                                                load_filaments.size();
                                record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
                                flush_and_exit(CLI_INVALID_PARAMS);
                            }
                    ...
                    if (clone_objects.size() > input_index) {
                        if (clone_objects[input_index] > 0) {
                            if (clone_objects[input_index] > MAX_CLONEABLE_SIZE) {
                                BOOST_LOG_TRIVIAL(error) << boost::format("invalid clone count %1% at index %2%, max %3%") %
                                                                clone_objects[input_index] % (input_index + 1) % MAX_CLONEABLE_SIZE;
                                record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
                                flush_and_exit(CLI_INVALID_PARAMS);
                            }
                ...
                if ((printer_technology != other_printer_technology) && (other_printer_technology != ptUnknown)) {
                    boost::nowide::cerr << "invalid printer_technology " << printer_technology << ", from source file " << file
                                        << std::endl;
                ...
                if (!config_substitutions.substitutions.empty()) {
                    ...
                    for (const ConfigSubstitution& subst : config_substitutions.substitutions)
                        BOOST_LOG_TRIVIAL(info) << "\tkey = \"" << subst.opt_def->opt_key << "\"\t old_value = \"" << subst.old_value
                                                << "\tnew_value = \"" << subst.new_value->serialize() << "\"\n";
                }
    ...
    if (!custom_gcode_file.empty()) {
        ...
        try {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("load custom_gcode from file %1% success, store custom gcodes to plate %2%") % file %
                                           (plate_id + 1);
    ...
    auto load_config_file = [config_substitution_rule](const std::string& file, DynamicPrintConfig& config, std::string& config_type,
        ...
        try {
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":load setting file " << file << ", with rule " << config_substitution_rule
                                    << std::endl;
            ...
            if (config_type == "machine") {
                ...
                boost::nowide::cerr << __FUNCTION__
                                    << boost::format(": unknown config type %1% of file %2% in load-settings") % config_type % file;
                return CLI_CONFIG_FILE_ERROR;
            }
    ...
    for (auto const& file : load_configs) {
        ...
        if (config_type == "machine") {
            ...
            if (!printer_model.empty()) {
                ...
                if (boost::filesystem::exists(printer_model_path)) {
                    ...
                    if (key_values.find("model_id") != key_values.end()) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                                << boost::format(":%1%, load printer_model_id %2% from current printer model %3%") %
                                                       __LINE__ % printer_model_id % printer_model;
                    }
                }
            }
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("loaded machine config %1%, type %2%, name %3%, inherits %4%, printer_model_id %5%") %
                                           file % config_name % config_from % new_printer_system_name % printer_model_id;
        } else if (config_type == "process") {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("loaded process config %1%, type %2%, name %3%, inherits %4%") % file % config_name %
                                           config_from % new_process_system_name;
        }
        ...
        for (int index = 0; index < load_filament_count; index++) {
            ...
            if (default_filament_file.empty() && !file.empty()) {
                ...
                if (config_type != "filament") {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                             << boost::format(": unknown config type %1% of file %2% in load-filaments") % config_type %
                                                    file;
                    record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                    flush_and_exit(CLI_CONFIG_FILE_ERROR);
                }
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("loaded default filament config %1%, type %2%, name %3%, inherits %4%") % file %
                                               config_from % config_name % default_filament_inherit;
                break;
            }
        }
        if ((load_filament_count > 0) && default_filament_file.empty()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                     << boost::format(": load_filament_count is %1%, but can not load a default filament") %
                                            load_filament_count;
            record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
            flush_and_exit(CLI_CONFIG_FILE_ERROR);
        }
    ...
    for (int index = 0; index < load_filament_count; index++) {
        ...
        if (!file.empty()) {
            ...
            if (config_type != "filament") {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(": unknown config type %1% of file %2% in load-filaments") % config_type % file;
                record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                flush_and_exit(CLI_CONFIG_FILE_ERROR);
            }
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("loaded filament %1% from file %2%, type %3%, name %4%, inherits %5%") % (index + 1) %
                                           file % config_from % config_name % inherits;
        } else {
            if (use_first_fila_as_default) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                        << boost::format(
                                               ": load filament %1% from default, config name %2%, filament_id %3%, current_index %4%") %
                                               (index + 1) % default_load_fila_name % default_load_fila_id % current_index;
            ...
            for (auto const& file : uptodate_configs) {
                ...
                if (config_type == "machine") {
                    if (config_name != current_printer_system_name) {
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("wrong machine config file %1% loaded, current machine config name %2% ") % config_name %
                                   current_printer_system_name;
                        record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                        flush_and_exit(CLI_CONFIG_FILE_ERROR);
                    }
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                                                   "load a machine config %1% from file %2%, upward_compatible_printers size is %3% ") %
                                                   config_name % file % upward_compatible_printers.size();
                    if (new_printer_name.empty() && !current_printer_system_name.empty()) {
                        ...
                        if (!printer_model.empty()) {
                            ...
                            if (boost::filesystem::exists(printer_model_path)) {
                                ...
                                if (key_values.find("model_id") != key_values.end()) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << __FUNCTION__
                                        << boost::format(":%1%, load printer_model_id %2% from current printer model %3%") % __LINE__ %
                                               printer_model_id % printer_model;
                                }
                            }
                        }
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << __FUNCTION__
                            << boost::format(":%1%, check printable size: old_printable_width=%2%, orig_printable_width=%3%, "
                                             "old_printable_depth=%4%, orig_printable_depth=%5%, old_printable_height=%6%, "
                                             "orig_printable_height=%7%") %
                                   __LINE__ % old_printable_width % orig_printable_width % old_printable_depth % orig_printable_depth %
                                   old_printable_height % orig_printable_height;
                        if ((orig_printable_width > 0) && (orig_printable_depth > 0) && (orig_printable_height > 0)) {
                            if ((old_printable_width > orig_printable_width) || (old_printable_depth > orig_printable_depth) ||
                                (old_printable_height > orig_printable_height)) {
                                std::string error_str =
                                    (boost::format("Printer Settings: the printable size {%1%, %2%, %3%} exceeds the default size.") %
                                     old_printable_width % old_printable_depth % old_printable_height)
                                        .str();
                    ...
                    if (config_name != current_process_system_name) {
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("wrong process config file %1% loaded, current process config name %2% ") % config_name %
                                   current_process_system_name;
                        record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                        flush_and_exit(CLI_CONFIG_FILE_ERROR);
                    }
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format("load a process config %1% from file %2%, current_print_compatible_printers size is %3% ") %
                               config_name % file % current_print_compatible_printers.size();
            ...
            if (new_printer_name.empty() && !current_printer_system_name.empty()) {
                ...
                if (!boost::filesystem::exists(system_printer_path)) {
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                               << boost::format(":%1%, can not find system preset file: %2% ") % __LINE__ %
                                                      system_printer_path;
                    // use original one
                } else {
                    DynamicPrintConfig config;
        ...
        if (load_filament_count == 0) {
            if (uptodate_filaments.size() > 0) {
                if (uptodate_filaments.size() != filament_count) {
                    BOOST_LOG_TRIVIAL(error) << boost::format("uptodate_filaments size %1% not equal to filament_count %2% ") %
                                                    uptodate_filaments.size() % filament_count;
                    record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
                    flush_and_exit(CLI_INVALID_PARAMS);
                }
                for (unsigned int index = 0; index < filament_count; index++) {
                    ...
                    if (config_name != current_filaments_system_name[index]) {
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("wrong filament config file %1% loaded, current filament config name %2%, index %3%") %
                                   config_name % current_filaments_system_name[index] % index;
                        record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                        flush_and_exit(CLI_CONFIG_FILE_ERROR);
                    }
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                                                   "load a filament config %1% from file %2%, using uptodate_filaments index %3%") %
                                                   config_name % file % index;
                }
            } else {
                current_index = 0;
                for (int index = 0; index < current_filaments_system_name.size(); index++) {
                    std::string system_filament_path = resources_dir() + "/profiles/BBL/filament_full/" +
                                                       current_filaments_system_name[index] + ".json";
                    current_index++;
                    if (!boost::filesystem::exists(system_filament_path)) {
                        BOOST_LOG_TRIVIAL(warning)
                            << __FUNCTION__
                            << boost::format(":%1%, can not find system preset file: %2% ") % __LINE__ % system_filament_path;
                        continue;
                    }
                    DynamicPrintConfig config;
                    std::string        config_type, config_name, filament_id, config_from;
                    ...
                    load_filaments_index.push_back(current_index);
                    load_filaments_inherit.push_back(config_name);
                    BOOST_LOG_TRIVIAL(info) << boost::format("LINE %4%: load a filament config %1% from file %2%, index %3%") %
                                                   config_name % system_filament_path % index % __LINE__;
                }
            }
        }
    } else if (is_bbl_3mf) {
    ...
    if (fetch_upward_values) {
        if (!current_printer_system_name.empty()) {
            ...
            if (!boost::filesystem::exists(system_printer_path)) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(":%1%, can not find system preset file: %2% ") % __LINE__ % system_printer_path;
                // skip
            } else {
                DynamicPrintConfig config;
                std::string        config_type, config_name, filament_id, config_from;
    ...
    if (fetch_compatible_values) {
        if (!current_process_system_name.empty()) {
            ...
            if (!boost::filesystem::exists(system_process_path)) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__
                                           << boost::format(":%1%, can not find system preset file: %2% ") % __LINE__ % system_process_path;
                // use original one
            } else {
                DynamicPrintConfig config;
                std::string        config_type, config_name, filament_id, config_from;
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("current printer %1%, new printer %2%, current process %3%, new process %4%") %
                                   current_printer_name % new_printer_name % current_process_name % new_process_name;
    BOOST_LOG_TRIVIAL(info)
        << boost::format("current printer inherits %1%, new printer inherits %2%, current process inherits %3%, new process inherits %4%") %
    ...
    for (int index = 0; index < current_print_compatible_printers.size(); index++) {
        BOOST_LOG_TRIVIAL(info) << boost::format("index %1%, current print compatible printer %2%") % index %
                                       current_print_compatible_printers[index];
    }
    for (int index = 0; index < new_print_compatible_printers.size(); index++) {
        BOOST_LOG_TRIVIAL(info) << boost::format("index %1%, new print compatible printer %2%") % index %
                                       new_print_compatible_printers[index];
    }
    ...
    if (!new_printer_name.empty()) {
        if (!new_process_name.empty()) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format(
                                           "new printer %1%, inherited from %2%, new process %3%, inherited from %4% ,compatible %5%") %
                                           new_printer_name % new_printer_system_name % new_process_name % new_process_system_name %
                                           process_compatible;
        } else {
            for (int index = 0; index < current_print_compatible_printers.size(); index++) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format(
                                           "new printer %1%, inherited from %2%, old process %3%, inherited from %4% ,compatible %5%") %
                                           new_printer_name % new_printer_system_name % current_process_name % current_process_system_name %
                                           process_compatible;
        }
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format(
                                       "old printer %1%, inherited from %2%, new process %3%, inherited from %4% ,compatible %5%") %
                                       current_printer_name % current_printer_system_name % new_process_name % new_process_system_name %
                                       process_compatible;
    } else {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format(
                                       "old printer %1%, inherited from %2%, old process %3%, inherited from %4% ,compatible %5%") %
                                       current_printer_name % current_printer_system_name % current_process_name %
                                       current_process_system_name % process_compatible;
    }
    if (!process_compatible && !new_printer_name.empty() && !current_printer_name.empty() && (new_printer_name != current_printer_name)) {
        ...
        if (upward_compatible_printers.size() > 0) {
            ...
            if (!process_compatible) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                         << boost::format(
                                                " %1% : current 3mf file not support the new printer %2%, new_printer_system_name %3%") %
                                                __LINE__ % new_printer_name % new_printer_system_name;
        ...
        /*else {
            BOOST_LOG_TRIVIAL(error) <<__FUNCTION__ << boost::format(" %1%: current 3mf file not support upward_compatible_printers, can not
        change machine preset.")%__LINE__; record_exit_reson(outfile_dir, CLI_3MF_NOT_SUPPORT_MACHINE_CHANGE, 0,
        cli_errors[CLI_3MF_NOT_SUPPORT_MACHINE_CHANGE], sliced_info); flush_and_exit(CLI_3MF_NOT_SUPPORT_MACHINE_CHANGE);
        }*/
    }
    ...
    if (is_bbl_3mf && machine_switch) {
        // we need to update the compatible printer and create a new process here, or if we load the 3mf in studio, the process preset can
        // not be loaded as not compatible
        ...
        if (current_preset) {
            *new_preset                                   = *current_preset;
            std::vector<std::string>& compatible_printers = new_preset->config.option<ConfigOptionStrings>("compatible_printers", true)
                                                                ->values;
    ...
    auto update_full_config = [](DynamicPrintConfig& full_config, const DynamicPrintConfig& config, std::set<std::string>& diff_key_sets,
                                 bool update_all = false, bool skip_gcodes = false) {
    ...
    if (!new_printer_name.empty() || up_config_to_date) {
        ...
        BOOST_LOG_TRIVIAL(info) << boost::format("update printer config to newest, different size %1%, different_settings: %2%") %
                                       different_keys_set.size() % different_settings[filament_count + 1];
        ...
        if (new_printer_name.empty()) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("no new printer, only update the different key, new different_settings: %1%") %
                                           different_settings[filament_count + 1];
        } else {
            ret = update_full_config(m_print_config, load_machine_config, different_keys_set, true);
            BOOST_LOG_TRIVIAL(info) << boost::format("load a new printer, update all the keys, different_settings: %1%") %
                                           different_settings[filament_count + 1];
            if (new_printer_name != current_printer_name) {
                // printer safe check
                BOOST_LOG_TRIVIAL(info)
                    << boost::format("check printer cli safe params, current_printer_name %1%, new_printer_name %2%, printer_model %3%") %
                           current_printer_name % new_printer_name % printer_model;
                std::map<std::string, std::string> printer_params;
                ...
                if (!fs::exists(directory_path)) {
                    ...
                    try {
                        ...
                        if (root_json.contains("printer")) {
                            ...
                            if (!printer_model.empty() && printer_json.contains(printer_model)) {
                                ...
                                if (printer_model_json.contains("machine_limits")) {
                                    ...
                                    for (auto param_iter = printer_params.begin(); param_iter != printer_params.end(); param_iter++) {
                                        ...
                                        if (option) {
                                            ...
                                            for (unsigned int index = 0; index < array_count; index++) {
                                                if ((index < new_array_count) && new_option.values[index] != 0.f &&
                                                    (new_option.values[index] < option->values[index])) {
                                                    BOOST_LOG_TRIVIAL(info) << boost::format("set key %1%  index %2%, from %3%  to %4%") %
                                                                                   key % index % option->values[index] %
                                                                                   new_option.values[index];
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("can not find machine_limits for printer %1% in cli_config.json") % printer_model;
                                }
                            } else {
                        ...
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__ << ": parse file " << cli_config_file << " got a generic exception, reason = " << err.what();
                    }
                }
            }
        }
    ...
    std::vector<std::string>& print_compatible_printers = m_print_config.option<ConfigOptionStrings>("print_compatible_printers", true)
                                                              ->values;
    if (!new_process_name.empty() || up_config_to_date) {
        ...
        std::set<std::string> different_keys_set(different_keys.begin(), different_keys.end());
        BOOST_LOG_TRIVIAL(info) << boost::format("update process config to newest, different size %1%, different_settings: %2%") %
                                       different_keys_set.size() % different_settings[0];
        ...
        if (new_process_name.empty()) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("no new process, only update the different key, new different_settings: %1%") %
                                           different_settings[0];
        } else {
            ret = update_full_config(m_print_config, load_process_config, different_keys_set, true);
            BOOST_LOG_TRIVIAL(info) << boost::format("load a new process, update all the keys, different_settings: %1%") %
                                           different_settings[0];
        }
    ...
    if ((load_filament_count > 0) || (up_config_to_date)) {
        for (int index = 0; index < load_filaments_config.size(); index++) {
            ...
            if (load_filament_count > 0) {
                ConfigOptionStrings* opt_filament_settings = static_cast<ConfigOptionStrings*>(
                    m_print_config.option("filament_settings_id", true));
            ...
            int                   diff_keys_size = different_keys_set.size();
            BOOST_LOG_TRIVIAL(info) << boost::format(
                                           "update filament %1%'s config to newest, different size %2%, name %3%, different_settings %4%") %
                                           filament_index % different_keys_set.size() % load_filaments_name[index] %
                                           different_settings[filament_index];
            for (const t_config_option_key& opt_key : config.keys()) {
                if ((load_filament_count == 0) && !different_keys_set.empty()) {
                    ...
                    if (iter != different_keys_set.end()) {
                        if (skip_modified_gcodes && (gcodes_key_set.find(opt_key) != gcodes_key_set.end())) {
                            ...
                            BOOST_LOG_TRIVIAL(info) << boost::format("%1%, filament %2%'s gcodes %3% modified, reset to default.") %
                                                           __LINE__ % filament_index % opt_key;
                        } else {
                            // uptodate, diff keys, continue
                            BOOST_LOG_TRIVIAL(info) << boost::format("%1%, keep key %2%") % __LINE__ % opt_key;
                            continue;
                ...
                if (source_opt == nullptr) {
                    // The key was not found in the source config, therefore it will not be initialized!
                    BOOST_LOG_TRIVIAL(error) << boost::format("can not find %1% from filament %2%: %3%") % opt_key % filament_index %
                                                    load_filaments_name[index];
                    record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                    flush_and_exit(CLI_CONFIG_FILE_ERROR);
                }
                if (source_opt->is_scalar()) {
                    if (opt_key == "compatible_printers_condition") {
                        ...
                        BOOST_LOG_TRIVIAL(info) << boost::format("skip scalar option %1% from filament %2%: %3%") % opt_key %
                                                       filament_index % load_filaments_name[index];
                        ;
                        continue;
                    }
                } else {
                    ...
                    if (opt == nullptr) {
                        ...
                        BOOST_LOG_TRIVIAL(error) << boost::format("can not create option %1% to config, from filament %2%: %3%") % opt_key %
                                                        filament_index % load_filaments_name[index];
                        record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                        flush_and_exit(CLI_CONFIG_FILE_ERROR);
                    }
            ...
            if (diff_keys_size != different_keys_set.size()) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("filament %1% new different key size %2%, different_settings %3%") %
                                               filament_index % different_keys_set.size() % different_settings[filament_index];
            }
        }
    }
    ...
    if (project_filament_colors_option &&
        (selected_filament_colors_option || !m_print_config.option<ConfigOptionFloats>("flush_volumes_matrix"))) {
            ...
            if (disable_wipe_tower_after_mapping) {
                ...
                for (unsigned int color_index = 0; color_index < selected_filament_colors.size(); color_index++) {
                    if (!selected_filament_colors[color_index].empty()) {
                        ...
                        BOOST_LOG_TRIVIAL(info) << boost::format("add color, index %1%, value %2% to set") % color_index %
                                                       selected_filament_colors[color_index];
                    } else
                ...
                if (filament_color_set.size() > 1) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                        "different filament colours,  switch disable_wipe_tower_after_mapping back to false");
                } else {
                    BOOST_LOG_TRIVIAL(warning) << boost::format(
                                                      "only %1% filament colour,  finally set disable_wipe_tower_after_mapping to true") %
                                                      filament_color_set.size();
                }
            }
        }
        ...
        if (project_filament_count > 0) {
            for (size_t index = 0; index < project_filament_count; index++) {
                BOOST_LOG_TRIVIAL(info) << boost::format("project filament %1% original color %2%") % index %
                                               project_filament_colors[index];
                if (selected_filament_colors.size() > index) {
                    if (!selected_filament_colors[index].empty()) {
                        ...
                        if ((ori_rgb_color[0] != new_rgb_color[0]) || (ori_rgb_color[1] != new_rgb_color[1]) ||
                            (ori_rgb_color[2] != new_rgb_color[2]) || (ori_rgb_color[3] != new_rgb_color[3])) {
            ...
            if (filament_is_support->size() != project_filament_count) {
                BOOST_LOG_TRIVIAL(error) << boost::format("filament_is_support's count %1% not equal to filament_colour's size %2%") %
...

            {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("flush_volumes_matrix before computing: %1%") %
                                               m_print_config.option<ConfigOptionFloats>("flush_volumes_matrix")->serialize();
            }
                ...
                for (int to_idx = 0; to_idx < project_filament_count; to_idx++) {
                    ...
                    if (from_idx == to_idx) {
                        ...
                        if (is_to_support) {
                            ...
                            // BOOST_LOG_TRIVIAL(info) << boost::format("src_idx %1%, src color %2%, dst idex %3%, dst color %4%")%from_idx
                            // %from_color %to_idx %to_color; BOOST_LOG_TRIVIAL(info) << boost::format("src_rgba {%1%,%2%,%3%,%4%} dst_rgba
                            // {%5%,%6%,%7%,%8%}")%(unsigned int)(from_rgb[0]) %(unsigned int)(from_rgb[1]) %(unsigned int)(from_rgb[2])
                            // %(unsigned int)(from_rgb[3])
                            //        %(unsigned int)(to_rgb[0]) %(unsigned int)(to_rgb[1]) %(unsigned int)(to_rgb[2]) %(unsigned
                            //        int)(to_rgb[3]);
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("flush_volumes_matrix after computed: %1%") %
                                           m_print_config.option<ConfigOptionFloats>("flush_volumes_matrix")->serialize();
        } else {
        ...
        for (gcodes_iter = m_models[0].plates_custom_gcodes.begin(); gcodes_iter != m_models[0].plates_custom_gcodes.end(); gcodes_iter++) {
            ...
            while (item_iter != gcode_info.gcodes.end()) {
                if (item_iter->type == CustomGCode::Custom) {
                    BOOST_LOG_TRIVIAL(warning) << boost::format("skip_modified_gcodes: remove custom gcodes %1% in plate %2%") %
                                                      item_iter->extra % gcodes_iter->first;
    ...
    if (disable_wipe_tower_after_mapping) {
        if (is_smooth_timelapse) {
            disable_wipe_tower_after_mapping = false;
            BOOST_LOG_TRIVIAL(warning) << boost::format(
                                              "%1%: disable_wipe_tower_after_mapping: set back to false due to smooth timelapse!") %
                                              __LINE__;
        } else {
            ConfigOptionBool* enable_wipe_op = m_print_config.option<ConfigOptionBool>("enable_prime_tower", true);
            enable_wipe_op->value            = false;
            BOOST_LOG_TRIVIAL(warning) << boost::format(
                                              "%1%: disable_wipe_tower_after_mapping: disable prime tower for only one filament!") %
                                              __LINE__;
    ...
    if (is_bbl_3mf && (old_printable_width > 0) && (old_printable_depth > 0) && (old_printable_height > 0)) {
        ...
        // if ((old_printable_width > current_printable_width) || (old_printable_depth > current_printable_depth) || (old_printable_height >
        // current_printable_height))
        if ((old_printable_width > current_printable_width) || (old_printable_depth > current_printable_depth) ||
            (old_printable_height > current_printable_height)) {
            BOOST_LOG_TRIVIAL(error) << boost::format("old printable size {%1%, %2%, %3%} is larger than new printable size {%4%, %5%, "
                                                      "%6%}, the object size should be limited") %
                                            old_printable_width % old_printable_depth % old_printable_height % current_printable_width %
                                            current_printable_depth % current_printable_height;
            ...
            BOOST_LOG_TRIVIAL(info)
                << boost::format(
                       "old printable size {%1%, %2%, %3%} is smaller than new printable size {%4%, %5%, %6%}, need to center the model") %
                       old_printable_width % old_printable_depth % old_printable_height % current_printable_width %
                       current_printable_depth % current_printable_height;
            shrink_to_new_bed = 1;
        } else {
            if ((current_exclude_area.size() > 0) && (current_exclude_area != old_exclude_area)) {
                BOOST_LOG_TRIVIAL(info) << boost::format("old printable size {%1%, %2%, %3%}, new printable size {%4%, %5%, %6%}, "
                                                         "exclude_area different, need to shrink") %
                                               old_printable_width % old_printable_depth % old_printable_height % current_printable_width %
                                               current_printable_depth % current_printable_height;
                shrink_to_new_bed = 2;
            } else {
                BOOST_LOG_TRIVIAL(info)
                    << boost::format(
                           "old printable size {%1%, %2%, %3%}, new printable size {%4%, %5%, %6%}, extract the same, no need shrink") %
                           old_printable_width % old_printable_depth % old_printable_height % current_printable_width %
                           current_printable_depth % current_printable_height;
            }
        }
    ...
    if (m_models.size() > 0) {
        BOOST_LOG_TRIVIAL(info) << boost::format("translate_old %1%, shrink_to_new_bed %2%, old bed size {%3%, %4%, %5%}") % translate_old %
                                       shrink_to_new_bed % old_printable_width % old_printable_depth % old_printable_height;
        if (translate_old) {
            BOOST_LOG_TRIVIAL(info) << boost::format("translate old 3mf, switch to older bed size,{%1%, %2%, %3%}") %
                                           (old_printable_width + bed3d_ax3s_default_tip_radius) %
                                           (old_printable_depth + bed3d_ax3s_default_tip_radius) % old_printable_height;
            partplate_list.reset_size(old_printable_width + bed3d_ax3s_default_tip_radius,
                                      old_printable_depth + bed3d_ax3s_default_tip_radius, old_printable_height, false);
        } else {
    ...
    auto check_plate_wipe_tower = [get_print_sequence, is_smooth_timelapse](Slic3r::GUI::PartPlate* plate, int plate_index,
                                                                            DynamicPrintConfig&    print_config,
                                                                            plate_obj_size_info_t& plate_obj_size_info) {
        plate_obj_size_info.obj_bbox = plate->get_objects_bounding_box();
        BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%, object bbox: min {%2%, %3%, %4%} - max {%5%, %6%, %7%}") % (plate_index + 1) %
                                       plate_obj_size_info.obj_bbox.min.x() % plate_obj_size_info.obj_bbox.min.y() %
                                       plate_obj_size_info.obj_bbox.min.z() % plate_obj_size_info.obj_bbox.max.x() %
                                       plate_obj_size_info.obj_bbox.max.y() % plate_obj_size_info.obj_bbox.max.z();
        ...
        Vec3d end(origin(0) + plate_obj_size_info.wipe_x + plate_obj_size_info.wipe_width + brim_width,
                  origin(1) + plate_obj_size_info.wipe_y + plate_obj_size_info.wipe_depth, 0.f);
    ...
    auto translate_models = [translate_old, shrink_to_new_bed, old_printable_width, old_printable_depth, old_printable_height,
                             current_printable_width, current_printable_depth, current_printable_height, current_exclude_area,
                             &plate_obj_size_infos](Slic3r::GUI::PartPlateList& plate_list, DynamicPrintConfig& print_config) {
        ...
        if (translate_old) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("translate old 3mf, switch back to current bed size,{%1%, %2%, %3%}") %
                                           old_printable_width % old_printable_depth % old_printable_height;
        ...
        if (shrink_to_new_bed > 0) {
            ...
            for (int index = 0; index < plate_count; index++) {
                ...
                if (shrink_to_new_bed == 1) {
                    Vec3d cur_center_offset{((double) old_printable_width) / 2, ((double) old_printable_depth) / 2, 0},
                        new_center_offset{((double) current_printable_width) / 2, ((double) current_printable_depth) / 2, 0};
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed 1, plate %1%, cur_origin: {%2%, %3%}, new_origin: {%4%, "
                                                             "%5%}, cur_center {%6%, %7%} new_center {%8%, %9%}") %
                                                   (index + 1) % cur_origin(0) % cur_origin(1) % new_origin(0) % new_origin(1) %
                                                   cur_center(0) % cur_center(1) % new_center(0) % new_center(1);
                } else {
                    // center the object
                    Vec3d          new_center_offset{((double) current_printable_width + exclude_width) / 2,
                                            ((double) current_printable_depth + exclude_depth) / 2, 0};
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed 2, plate %1%, new_origin: {%2%, %3%}, new_center: {%4%, "
                                                             "%5%}, obj bbox(including wipe tower) min {%6%, %7%} max {%8%, %9%}") %
                                                   (index + 1) % new_origin(0) % new_origin(1) % new_center(0) % new_center(1) %
                                                   bbox.min(0) % bbox.min(1) % bbox.max(0) % bbox.max(1);
                }
                offset(2) = 0.f;
                BOOST_LOG_TRIVIAL(info) << boost::format(
                                               "shrink_to_new_bed %1%, plate %2% translate offset: {%3%, %4%} wipe_offset {%5%, %6%}") %
                                               shrink_to_new_bed % (index + 1) % offset[0] % offset[1] % wipe_offset[0] % wipe_offset[1];
                cur_plate->translate_all_instance(offset);

                if (wipe_x_option) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed %4%, plate %1%: wipe tower src: {%2%, %3%}") % (index + 1) %
                                                   wipe_x_option->get_at(index) % wipe_y_option->get_at(index) % shrink_to_new_bed;
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed %4%, plate %1% wipe tower changes to: {%2%, %3%}") %
                                                   (index + 1) % wipe_x_option->get_at(index) % wipe_y_option->get_at(index) %
                                                   shrink_to_new_bed;
                }
            }
            BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed, shrink all the models to current bed size,{%1%, %2%, %3%}") %
                                           current_printable_width % current_printable_depth % current_printable_height;
    ...
    if (plate_data_src.size() > 0) {
        ...
        for (int index = 0; index < plate_count; index++) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%,  has_wipe_tower %2%, wipe_x %3%, wipe_y %4%, width %5%, depth %6%") %
                                           (index + 1) % plate_obj_size_infos[index].has_wipe_tower % plate_obj_size_infos[index].wipe_x %
                                           plate_obj_size_infos[index].wipe_y % plate_obj_size_infos[index].wipe_width %
                                           plate_obj_size_infos[index].wipe_depth;
        }
    ...
    if (downward_check) {
        ...
        for (auto const& file : downward_settings) {
            ...
            if ((config_type != "machine") || (config_from != "system")) {
                BOOST_LOG_TRIVIAL(info) << boost::format("found invalid config type %1% or from %2% in file %3% when downward_check") %
                                               config_type % config_from % file_path;
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("downward_check: printable size{%1%,%2%, %3%}, exclude area{%4%, %5%: %6% x %7%}") %
                                           ...
                                           printer_plate.exclude_x % printer_plate.exclude_y % printer_plate.exclude_width %
                                           printer_plate.exclude_depth;
    ...
    if (downward_check_size > 0) {
        ...
        for (int index = 0; index < plate_count; index++) {
            ...
            for (int index2 = 0; index2 < downward_check_size; index2++) {
                ...
                if ((size.z() > plate_info.printable_height) || (size.y() > plate_info.printable_depth) ||
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format(
                               "plate %1%, downward_check index %2%, name %3%, bbox {%4%, %5%, %6%} exceeds printer size {%7%, %8%, %9%}") %
                               (index + 1) % (index2 + 1) % plate_info.printer_name % size.x() % size.y() % size.z() %
                               plate_info.printable_width % plate_info.printable_depth % plate_info.printable_height;
                ...
                if (plate_info.exclude_width > 0) {
                    ...
                    if ((size.x() > real_width) && (size.y() > real_depth)) {
                        BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%, downward_check index %2%, name %3%, bbox {%4%, %5%} exceeds "
                                                                 "real size without exclude_area {%6%, %7%}") %
                                                       (index + 1) % (index2 + 1) % plate_info.printer_name % size.x() % size.y() %
                                                       real_width % real_depth;
                        downward_check_status[index2] = true;
                        failed_count++;
                        continue;
                    }
        ...
        if (failed_count < downward_check_size) {
            // has success ones
            BOOST_LOG_TRIVIAL(info) << boost::format("downward_check: downward_check_size %1%, failed_count %2%") % downward_check_size %
                                           failed_count;
            for (int index2 = 0; index2 < downward_check_size; index2++) {
                if (downward_check_status[index2])
                    continue;
        ...
        if (opt_key == "assemble") {
            ...
            if (repetitions_count <= 1) {
                ...
                if (plate_to_slice == 0) {
                    ...
                    BOOST_LOG_TRIVIAL(error) << boost::format("Invalid params:invalid plate %1% to slice, total %2%") % plate_to_slice %
                                                    partplate_list.get_plate_count();
                    record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
                    flush_and_exit(CLI_INVALID_PARAMS);
                }
            ...
            if (orient_option == 0) {
                ...
                for (auto& model : m_models)
                    for (ModelObject* o : model.objects) {
                        orients_requirement.insert(std::pair<size_t, bool>(o->id().id, false));
                        BOOST_LOG_TRIVIAL(info)
                            << "object " << o->name << ", id :" << o->id().id << ", no need to orient when setting orient to 0\n";
                    }
                ...
                for (auto& model : m_models)
                    for (ModelObject* o : model.objects) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << "object " << o->name << ", id :" << o->id().id << ", need to orient when setting orient to 1\n";
                    }
            } else {
            ...
            for (auto& model : m_models) {
                const bool all_objects_have_instances = std::none_of(model.objects.begin(), model.objects.end(),
    ...
    if ((plate_to_slice < 0) || (plate_to_slice > partplate_list.get_plate_count())) {
        ...
        if (curr_plate_seq == PrintSequence::ByDefault) {
            ...
            if (seq_print && (seq_print->value == PrintSequence::ByObject)) {
                BOOST_LOG_TRIVIAL(info)
                    << boost::format("Check whether need to arrange by print_sequence: plate %1% print by object, set from global") %
                           plate_to_slice;
                is_seq_print_for_curr_plate = true;
            }
        } else if (curr_plate_seq == PrintSequence::ByObject) {
            BOOST_LOG_TRIVIAL(info)
                << boost::format("Check whether need to arrange by print_sequence: plate %1% print by object, set from plate self") %
                       plate_to_slice;
            is_seq_print_for_curr_plate = true;
        }
    ...
    if ((!need_arrange) && is_bbl_3mf && !shrink_to_new_bed && (plate_to_slice > 0)) {
        if (((old_height_to_rod != 0.f) && (old_height_to_rod != height_to_rod)) ||
            ...
            if (is_seq_print_for_curr_plate) {
                need_arrange = true;
                BOOST_LOG_TRIVIAL(info) << boost::format("old_height_to_rod %1%, old_height_to_lid %2%,  old_max_radius %3%, current "
                                                         "height_to_rod %4%, height_to_lid %5%, clearance_radius %6%, need arrange!") %
                                               old_height_to_rod % old_height_to_lid % old_max_radius % height_to_rod % height_to_lid %
                                               clearance_radius;
            }
        }
    }
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("before arrange, need_arrange=%1%, duplicate_count %2%, filament_color_changed %3%") %
                                   need_arrange % duplicate_count % filament_color_changed;
        ...
        for (int index = 0; index < partplate_list.get_plate_count(); index++) {
            ...
            if (plate_data_src.size() > index) {
                if (!plate_data_src[index]->thumbnail_file.empty()) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded thumbnail %2%.") % (index + 1) %
                                                   plate_data_src[index]->thumbnail_file;
                    plate_data_src[index]->thumbnail_file.clear();
                }
                if (!plate_data_src[index]->no_light_thumbnail_file.empty()) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded no_light_thumbnail %2%.") % (index + 1) %
                ...
                if (!plate_data_src[index]->top_file.empty()) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded top_thumbnail %2%.") % (index + 1) %
                                                   plate_data_src[index]->top_file;
                    plate_data_src[index]->top_file.clear();
                }
                if (!plate_data_src[index]->pick_file.empty()) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded pick_thumbnail %2%.") % (index + 1) %
                                                   plate_data_src[index]->pick_file;
    ...
    if (!assemble_plate_info_list.empty()) {
        ...
        if (plate_count != partplate_list.get_plate_count()) {
            BOOST_LOG_TRIVIAL(error) << boost::format("mismatch plate count, to_assemble %1%, generated %2%") % plate_count %
                                            partplate_list.get_plate_count();
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
        }
        ...
        for (size_t i = 0; i < plate_count; i++) {
            ...
            Slic3r::GUI::PartPlate* cur_plate      = (Slic3r::GUI::PartPlate*) partplate_list.get_plate(i);
            BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%, need arrange %2%, filaments_count %3%") % (i + 1) %
                                           assemble_plate.need_arrange % assemble_plate.filaments_count;
            if (assemble_plate.need_arrange) {
                ...
                for (size_t oidx = 0; oidx < plate_obj_count; ++oidx) {
                    ...
                    for (size_t inst_idx = 0; inst_idx < mo->instances.size(); ++inst_idx) {
                        ...
                        BOOST_LOG_TRIVIAL(debug)
                            << boost::format("plate %1%: add object %2%  object index %3%, into selected") % (i + 1) % ap.name % (oidx + 1);
                    }
                }
                ...
                if (!arrange_cfg.is_seq_print && assemble_plate.filaments_count > 1) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("prime_tower_width %1% wipe_tower_rotation_angle %2% prime_volume %3%") %
                                                   width_option->value % rotation_angle_option->value % volume_option->value;

                    wipe_x_option->set_at(&wt_x_opt, i, 0);
                    wipe_y_option->set_at(&wt_y_opt, i, 0);

                    ArrangePolygon wipe_tower_ap = cur_plate->estimate_wipe_tower_polygon(m_print_config, i, assemble_plate.filaments_count,
                                                                                          true);
                ...
                {
                    BOOST_LOG_TRIVIAL(debug) << "arrange bedpts:" << beds[0].transpose() << ", " << beds[1].transpose() << ", "
                                             << beds[2].transpose() << ", " << beds[3].transpose();
                ...
                for (ArrangePolygon& ap : selected) {
                    ...
                    if (ap.bed_idx != i) {
                        BOOST_LOG_TRIVIAL(error)
                            << __FUNCTION__
                            << boost::format(":arrange failed: ap.name %1% ap.bed_idx %2%, plate index %3%") % ap.name % ap.bed_idx % i;
                ...
                for (size_t oidx = 0; oidx < plate_obj_count; ++oidx) {
                    ...
                    BOOST_LOG_TRIVIAL(debug) << boost::format("plate %1%: no arrange, directly translate object %2%  by {%3%, %4%}") %
                                                    (i + 1) % mo->name % plate_origin(0) % plate_origin(1);
                }
            }
        }
        ...
        if (m_models.size() > 0) {
            ...
            while (!finished_arrange) {
                ...
                if (duplicate_count > 0) {
                    ...
                    if (!first_run) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("restore model object and plate, new duplicate_count %1%, arrange_count=%2%") %
                                   duplicate_count % arrange_count;
                        ...
                        if (plate_to_slice > 0) {
                            ...
                            if (wipe_x_option && (wipe_x_option->size() > (plate_to_slice - 1))) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << boost::format("%1%, plate_to_slice %2%, orig_wipe_x=%3%") % __LINE__ % plate_to_slice % orig_wipe_x;
                            }
                            if (wipe_y_option && (wipe_y_option->size() > (plate_to_slice - 1))) {
                                orig_wipe_y = wipe_y_option->get_at(plate_to_slice - 1);
                                BOOST_LOG_TRIVIAL(info)
                                    << boost::format("%1%, plate_to_slice %2%, orig_wipe_y=%3%") % __LINE__ % plate_to_slice % orig_wipe_y;
                            }
                        }
                    }
                ...
                if ((duplicate_count == 0) && (plate_to_slice == 0)) {
                    ...
                    for (size_t oidx = 0; oidx < model.objects.size(); ++oidx) {
                        ...
                        for (size_t inst_idx = 0; inst_idx < mo->instances.size(); ++inst_idx) {
                            ...
                            if (!locked) {
                                ...
                                boost::nowide::cout
                                    << __FUNCTION__
                                    << boost::format(": skip locked instance, obj_id %1%, instance_id %2%") % oidx % inst_idx;
                            }
                        }
                    }
                    ...
                    if (used_filament_set.size() > 0) {
                        ...
                        ConfigOptionFloats* wipe_y_option         = m_print_config.option<ConfigOptionFloats>("wipe_tower_y", true);
                        ConfigOptionFloat*  width_option          = m_print_config.option<ConfigOptionFloat>("prime_tower_width", true);
                        ConfigOptionFloat*  rotation_angle_option = m_print_config.option<ConfigOptionFloat>("wipe_tower_rotation_angle",
                                                                                                             true);
                        ConfigOptionFloat*  volume_option         = m_print_config.option<ConfigOptionFloat>("prime_volume", true);
                        ...
                        for (int bedid = 0; bedid < MAX_PLATE_COUNT; bedid++) {
                            ...
                            ArrangePolygon wipe_tower_ap = partplate_list.get_plate(plate_index_valid)
                                                               ->estimate_wipe_tower_polygon(m_print_config, plate_index_valid,
                                                                                             extruder_size, true);
                    ...
                    for (size_t oidx = 0; oidx < model.objects.size(); ++oidx) {
                        ...
                        for (size_t inst_idx = 0; inst_idx < mo->instances.size(); ++inst_idx) {
                            ...
                            ArrangePolygons& cont   = mo->instances[inst_idx]->printable ? (in_plate ? selected : unselected) : unprintable;
                            bool             locked = partplate_list.preprocess_arrange_polygon_other_locked(oidx, inst_idx, ap, in_plate);
                            BOOST_LOG_TRIVIAL(info) << boost::format("name %4% in_plate %1% printable %2%, locked %3%") % in_plate %
                                                           mo->instances[inst_idx]->printable % locked % ap.name;
                            if (!locked) {
                                ap.itemid = cont.size();
                                ...
                                BOOST_LOG_TRIVIAL(debug)
                                    << __FUNCTION__
                                    << boost::format("arrange: skip locked instance, obj_id %1%, name %2%") % oidx % mo->name;
                            }
                        }
                    }
                    ...
                    if (m_print_config.has("wipe_tower_x") && (is_smooth_timelapse || !arrange_cfg.is_seq_print || (selected.size() <= 1))) {
                        ...
                        if ((filaments_cnt == 0) || need_skip) {
                            ...
                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__
                                << boost::format(
                                       "arrange: slice filaments info invalid or need_skip, get from partplate: filament_count %1%") %
                                       filaments_cnt;
                        }
                        ...
                        if ((filaments_cnt <= 1) && !is_smooth_timelapse) {
                            BOOST_LOG_TRIVIAL(warning)
                                << __FUNCTION__
                                << boost::format("arrange: not a multi-color object anymore, drop the wipe tower before arrange.");
                        } else {
                            ...
                            if (wipe_tower_brim_width_opt) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << __FUNCTION__ << boost::format("arrange wipe_tower: wp_brim_width %1%") % wp_brim_width;
                            }

                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__
                                << boost::format("arrange wipe_tower: x=%1%, y=%2%, width=%3%, depth=%4%, angle=%5%, prime_volume=%6%, "
                                                 "filaments_cnt=%7%, layer_height=%8%, plate_width=%9%, plate_depth=%10%") %
                            ...
                            if ((y + depth + margin + wp_brim_width) > (float) plate_depth) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << __FUNCTION__
                                    << boost::format("arrange wipe_tower: exceeds the border, change y to %1%, plate_depth=%2%") % y %
                                           plate_depth;
                            }
                            ...
                            if ((x + w + margin + wp_brim_width) > (float) plate_width) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << __FUNCTION__
                                    << boost::format("arrange wipe_tower: exceeds the border, change x to %1%, plate_width=%2%") % y %
                                           plate_width;
                            }
                            if (x < margin) {
                                x = margin;
                ...
                if (duplicate_count == 0) {
                    if (plate_to_slice > 0) {
                        ...
                        for (ArrangePolygon& ap : selected) {
                            ...
                            if (ap.bed_idx != (plate_to_slice - 1)) {
                                BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                                         << boost::format(":arrange failed: ap.name %1% ap.bed_idx %2%, plate index %3%") %
                                                                ap.name % ap.bed_idx % (plate_to_slice - 1);
                        ...
                        for (ArrangePolygon& ap : selected) {
                            ...
                            bed_idx_max = std::max(ap.bed_idx, bed_idx_max);
                            BOOST_LOG_TRIVIAL(trace) << "after arrange: name=" << ap.name
                                                     << boost::format(",bed_id %1%, trans {%2%,%3%}") % ap.bed_idx %
                    ...
                    // Apply the arrange result to unselected objects(due to the sukodu-style column changes, the position of unselected may
                    // also be modified)
                    ...
                    for (ArrangePolygon& ap : selected) {
                        ...
                        if (ap.bed_idx != (plate_to_slice - 1)) {
                            ...
                            if (!duplicate_single_object) {
                                BOOST_LOG_TRIVIAL(warning) << boost::format("arrange failed when duplicate multiple objects at count %1%, "
                                                                            "low_duplicate_count %2%, up_duplicate_count %3%") %
                                                                  duplicate_count % low_duplicate_count % up_duplicate_count;

                                if (duplicate_count == 1) {
                                    BOOST_LOG_TRIVIAL(warning)
                                        << __FUNCTION__
                                        << boost::format(": failed even on duplicate 1 copy, just print one original model");
                                    duplicate_count = 0;
                                } else {
                                    if (duplicate_count == low_duplicate_count) {
                                        BOOST_LOG_TRIVIAL(warning)
                                            << __FUNCTION__
                                            << boost::format(": previous success, but currently failed count %1%!!!") % duplicate_count;
                                    ...
                                    BOOST_LOG_TRIVIAL(warning)
                                        << __FUNCTION__
                                        << boost::format(": try new count %1%, low_duplicate_count %2%, up_duplicate_count %3%") %
                                               duplicate_count % low_duplicate_count % up_duplicate_count;
                                }
                                // record_exit_reson(outfile_dir, CLI_OBJECT_ARRANGE_FAILED, 0, cli_errors[CLI_OBJECT_ARRANGE_FAILED],
                                // sliced_info); flush_and_exit(CLI_OBJECT_ARRANGE_FAILED);
                                failed_this_time = true;
                                break;
                            }
                        } else {
                            BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                                    << boost::format(":arrange success: ap.name %1% ap.bed_idx %2%, plate index %3%") %
                                                           ap.name % ap.bed_idx % (plate_to_slice - 1);
                    ...
                    if (failed_this_time) {
                        if (duplicate_count == 0) {
                            ...
                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__
                                << boost::format(": restore to the original model and plates, orig_wipe_x %1%, orig_wipe_y %2%") %
                                       orig_wipe_x % orig_wipe_y;
                            finished_arrange = true;
                            model            = original_model;
                            partplate_list.load_from_3mf_structure(plate_data_src);
                            partplate_list.reset_size(current_printable_width, current_printable_depth, current_printable_height, true,
                                                      true);
                            if ((orig_wipe_x > 0.f) && (orig_wipe_y > 0.f)) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << __FUNCTION__
                                    << boost::format(": restore wipe_tower position to {%1%, %2%}") % orig_wipe_x % orig_wipe_y;
                            }
                    ...
                    if (duplicate_single_object) {
                        if (real_duplicate_count <= 1) {
                            BOOST_LOG_TRIVIAL(warning)
                                << boost::format("no object can be placed under single object mode, restore to the original model and "
                                                 "plates also, orig_wipe_x %1%, orig_wipe_y %2%") %
                                       orig_wipe_x % orig_wipe_y;
                            // record_exit_reson(outfile_dir, CLI_OBJECT_ARRANGE_FAILED, 0, cli_errors[CLI_OBJECT_ARRANGE_FAILED],
                            // sliced_info); flush_and_exit(CLI_OBJECT_ARRANGE_FAILED);
                            finished_arrange = true;
                        ...
                        BOOST_LOG_TRIVIAL(info) << __FUNCTION__
                                                << boost::format(": multiple objects mode, arrange success on count %1%, "
                                                                 "low_duplicate_count %2%, up_duplicate_count %3%") %
                                                       duplicate_count % low_duplicate_count % up_duplicate_count;
                        if ((duplicate_count == up_duplicate_count) || (duplicate_count == (up_duplicate_count - 1))) {
                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__ << boost::format(": found the max arrangeable count %1%") % duplicate_count;
                        } else {
                            low_duplicate_count = duplicate_count;
                            duplicate_count     = (up_duplicate_count + low_duplicate_count) / 2;
                            BOOST_LOG_TRIVIAL(info)
                                << __FUNCTION__
                                << boost::format(": try new count %1%, low_duplicate_count %2%, up_duplicate_count %3%") % duplicate_count %
                                       low_duplicate_count % up_duplicate_count;
                            continue;
                        }
                    }
    ...
    for (auto const& opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            BOOST_LOG_TRIVIAL(info) << "Need to slice for plate " << plate_to_slice << ", total plate count "
                                    << partplate_list.get_plate_count() << " partplates!" << std::endl;
#if defined(__linux__) || defined(__LINUX__)
            if (g_cli_callback_mgr.is_started()) {
            ...
            for (Model& model_in : m_models) {
                ...
                while (!finished) {
                    ...
                    for (int index = 0; index < partplate_list.get_plate_count(); index++) {
                        ...
                        if (count == 0) {
                            BOOST_LOG_TRIVIAL(error) << "plate " << index + 1
                                                     << ": Nothing to be sliced, Either the print is empty or no object is fully inside "
                                                        "the print volume before apply."
                                                     << std::endl;
                                ...
                                for (ModelInstance* i : model_object->instances) {
                                    ...
                                    if (skip_maps.find(i->loaded_id) != skip_maps.end()) {
                                        ...
                                        if (i->print_volume_state == ModelInstancePVS_Inside) {
                                            ...
                                            BOOST_LOG_TRIVIAL(info)
                                                << boost::format("Plate %1%: skip object %2%.") % (index + 1) % i->loaded_id;
                                            // need to regenerate the thumbnail
                                            if (plate_data_src.size() > index) {
                                                if (!plate_data_src[index]->thumbnail_file.empty()) {
                                                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded thumbnail %2%.") %
                                                                                   (index + 1) % plate_data_src[index]->thumbnail_file;
                                                    plate_data_src[index]->thumbnail_file.clear();
                                                }
                                                if (!plate_data_src[index]->no_light_thumbnail_file.empty()) {
                                                    BOOST_LOG_TRIVIAL(info)
                                                        << boost::format("Plate %1%: clear loaded no_light_thumbnail %2%.") % (index + 1) %
                                                               plate_data_src[index]->no_light_thumbnail_file;
                                                    plate_data_src[index]->no_light_thumbnail_file.clear();
                                                }
                                                if (!plate_data_src[index]->top_file.empty()) {
                                                    BOOST_LOG_TRIVIAL(info) << boost::format("Plate %1%: clear loaded top_thumbnail %2%.") %
                                                                                   (index + 1) % plate_data_src[index]->top_file;
                                                    plate_data_src[index]->top_file.clear();
                                                }
                                                if (!plate_data_src[index]->pick_file.empty()) {
                                                    BOOST_LOG_TRIVIAL(info)
                                                        << boost::format("Plate %1%: clear loaded pick_thumbnail %2%.") % (index + 1) %
                                                               plate_data_src[index]->pick_file;
                                                    plate_data_src[index]->pick_file.clear();
                                                }
                                            }
                                    ...
                                    if (i->print_volume_state == ModelInstancePVS_Partly_Outside) {
                                        BOOST_LOG_TRIVIAL(error) << "plate " << index + 1 << ": Found Object " << model_object->name
                                                                 << " partly inside, can not be sliced." << std::endl;
                                        record_exit_reson(outfile_dir, CLI_OBJECTS_PARTLY_INSIDE, index + 1,
                                                          cli_errors[CLI_OBJECTS_PARTLY_INSIDE], sliced_info);
                                        ...
                                        for (const ModelVolume* vol : model_object->volumes) {
                                            if (vol->is_model_part()) {
                                                ...
                                                BOOST_LOG_TRIVIAL(debug) << boost::format("volume triangle count %1%, total %2%") %
                                                                                volume_triangle_count % triangle_count;
                                                if ((max_triangle_count_per_plate != 0) && (triangle_count > max_triangle_count_per_plate)) {
                                                    BOOST_LOG_TRIVIAL(error)
                                                        << "plate " << index + 1 << ": triangle count " << triangle_count
                                                        << " exceeds the limit:" << max_triangle_count_per_plate;
                                                    record_exit_reson(outfile_dir, CLI_TRIANGLE_COUNT_EXCEEDS_LIMIT, index + 1,
                                                                      cli_errors[CLI_TRIANGLE_COUNT_EXCEEDS_LIMIT], sliced_info);
                            ...
                            if (printable_instances == 0) {
                                BOOST_LOG_TRIVIAL(error) << "plate " << index + 1 << ": Nothing to be sliced, after skipping "
                                                         << skipped_count << " objects." << std::endl;
                                record_exit_reson(outfile_dir, CLI_NO_SUITABLE_OBJECTS_AFTER_SKIP, index + 1,
                                                  cli_errors[CLI_NO_SUITABLE_OBJECTS_AFTER_SKIP], sliced_info);
                        ...
                        // BBS: TODO
                        // BOOST_LOG_TRIVIAL(info) << boost::format("print_volume {%1%,%2%,%3%}->{%4%, %5%, %6%}, has %7% printables") %
                        // print_volume.min(0) % print_volume.min(1)
                        ...
                        if (!err.string.empty()) {
                            if ((STRING_EXCEPT_LAYER_HEIGHT_EXCEEDS_LIMIT == err.type) && no_check) {
                                ...
                                int validate_error;
                                switch (err.type) {
                                case STRING_EXCEPT_FILAMENT_NOT_MATCH_BED_TYPE: validate_error = CLI_FILAMENT_NOT_MATCH_BED_TYPE; break;
                                case STRING_EXCEPT_FILAMENTS_DIFFERENT_TEMP: validate_error = CLI_FILAMENTS_DIFFERENT_TEMP; break;
                                case STRING_EXCEPT_OBJECT_COLLISION_IN_SEQ_PRINT: validate_error = CLI_OBJECT_COLLISION_IN_SEQ_PRINT; break;
                                case STRING_EXCEPT_OBJECT_COLLISION_IN_LAYER_PRINT:
                                    validate_error = CLI_OBJECT_COLLISION_IN_LAYER_PRINT;
                                    break;
                                default: validate_error = CLI_VALIDATE_ERROR; break;
                                }
                                if (no_check)
                        ...
                        if (print->empty()) {
                            ...
                            try {
                                ...
                                if (g_cli_callback_mgr.is_started()) {
                                    ...
                                    g_cli_callback_mgr.set_plate_info(index + 1,
                                                                      (plate_to_slice == 0) ? partplate_list.get_plate_count() : 1);
                                ...
                                if (!printer_model_string.empty()) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info) << boost::format("printer_model_string: %1%, is_bbl_vendor_preset %2%") %
                                                                   printer_model_string % is_bbl_vendor_preset;
                                } else {
                                    if (!new_printer_name.empty())
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format(
                                               "new_printer_name: %1%, current_printer_system_name %2%, is_bbl_vendor_preset %3%") %
                                               new_printer_name % current_printer_system_name % is_bbl_vendor_preset;
                                }
                                ...
                                if (printer_technology == ptFFF) {
                                    ...
                                    if (!conflict_result.empty()) {
                                        BOOST_LOG_TRIVIAL(error)
                                            << "plate " << index + 1 << ": found slicing result conflict!" << std::endl;
                                        record_exit_reson(outfile_dir, CLI_GCODE_PATH_CONFLICTS, index + 1,
                                                          cli_errors[CLI_GCODE_PATH_CONFLICTS], sliced_info);
                                    ...
                                    if (!g_slicing_warnings.empty()) {
                                        for (unsigned int i = 0; i < g_slicing_warnings.size(); i++) {
                                            ...
                                            if ((status.warning_step != -1) &&
                                                (status.message_type != PrintStateBase::SlicingDefaultNotification)) {
                                                sliced_plate_info.warning_message = status.text;

                                                if (status.warning_level == PrintStateBase::WarningLevel::NON_CRITICAL) {
                                                    BOOST_LOG_TRIVIAL(warning)
                                                        << "plate " << index + 1 << ": found NON_CRITICAL slicing warnings: " << status.text
                                                        << std::endl;
                                                } else {
                                                    BOOST_LOG_TRIVIAL(warning)
                                                        << boost::format("plate %1%: found slicing warnings: %2%, no_check=%3%") %
                                                               (index + 1) % status.text % no_check;
                                                    if (!no_check) {
                                                        ...
                                                        if (status.message_type == PrintStateBase::SlicingEmptyGcodeLayers ||
                                                            ...
                                                            record_exit_reson(outfile_dir, CLI_SLICING_ERROR, index + 1,
                                                                              cli_errors[CLI_SLICING_ERROR], sliced_info);
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << "export_gcode finished: time_using_cache update to " << time_using_cache << " secs.";
                                    ...
                                    // { return this->render_thumbnails(params); });
                                     ...
                                     // We need to finalize the filename beforehand because the export function sets the filename inside the
                                 zip metadata outfile_final = sla_print.print_statistics().finalize_output_path(outfile);
                                     sla_archive.export_print(outfile_final, sla_print);
                                 }*/
                                /*if (outfile != outfile_final) {
                                    if (Slic3r::rename_file(outfile, outfile_final)) {
                                        boost::nowide::cerr << "Renaming file " << outfile << " to " << outfile_final << " failed" <<
                                std::endl; record_exit_reson(outfile_dir, 1, index+1, cli_errors[1], sliced_info); flush_and_exit(1);
                                    }
                                ...
                                if (export_slicedata) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << "plate " << index + 1 << ":will export Slicing data to " << export_slice_data_dir;
                                ...
                                if (max_slicing_time_per_plate != 0) {
                                    ...
                                    if (time_cost > max_slicing_time_per_plate) {
                                        sliced_plate_info.warning_message =
                                            (boost::format("plate %1%'s slice time %2% exceeds the limit %3%, return error.") %
                                             (index + 1) % time_cost % max_slicing_time_per_plate)
                                                .str();
                                        BOOST_LOG_TRIVIAL(error) << sliced_plate_info.warning_message;
                                        sliced_info.sliced_plates.push_back(sliced_plate_info);
                                        record_exit_reson(outfile_dir, CLI_SLICING_TIME_EXCEEDS_LIMIT, index + 1,
                                                          cli_errors[CLI_SLICING_TIME_EXCEEDS_LIMIT], sliced_info);
    ...
    if (export_to_3mf) {
        ...
        if (sliced_plate == -1) {
            ...
            for (int i = 0; i < plate_data_list.size(); i++) {
                if (skip_useless_pick && (plate_object_count[i] == 1)) {
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                                                   "only has 1 object,  set plate %1%'s is_label_object_enabled from %2% to false") %
                                                   (i + 1) % (plate_data_list[i]->is_label_object_enabled);
                    plate_data_list[i]->is_label_object_enabled = false;
                }
            }
        } else {
            if (skip_useless_pick && (plate_object_count[sliced_plate - 1] == 1)) {
                BOOST_LOG_TRIVIAL(info) << boost::format("only has 1 object,  set plate %1%'s is_label_object_enabled from %2% to false") %
                                               sliced_plate % (plate_data_list[sliced_plate - 1]->is_label_object_enabled);
        ...
        const ConfigOptionFloats*  nozzle_diameter_option = dynamic_cast<const ConfigOptionFloats*>(
            m_print_config.option("nozzle_diameter"));
        ...
        for (int i = 0; i < plate_data_list.size(); i++) {
            ...
            if (!plate_data->plate_thumbnail.is_valid()) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format(
                                               "thumbnails stage: plate %1%'s thumbnail data is invalid, check the file %2% exist or not") %
                ...
                if (plate_data->thumbnail_file.empty() || (!boost::filesystem::exists(plate_data->thumbnail_file))) {
                    ...
                    if (regenerate_thumbnails) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s thumbnail file %2% cleared, need to regenerate") % (i + 1) %
                                   plate_data->thumbnail_file;
                        plate_data->thumbnail_file.clear();
                    } else
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s thumbnail file exists, no need to regenerate") % (i + 1);
                }
            } else {
                if (regenerate_thumbnails)
            ...
            if (plate_data->no_light_thumbnail_file.empty()) {
                ...
                if (plate_data->no_light_thumbnail_file.empty() || (!boost::filesystem::exists(plate_data->no_light_thumbnail_file))) {
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format("thumbnails stage: plate %1%'s no_light_thumbnail_file %2% also not there, need to regenerate") %
                    ...
                    if (regenerate_thumbnails) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s no_light_thumbnail file %2% cleared, need to regenerate") %
                                   (i + 1) % plate_data->no_light_thumbnail_file;
                        plate_data->no_light_thumbnail_file.clear();
                    } else
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s no_light_thumbnail file exists, no need to regenerate") %
                                   (i + 1);
                }
            }
            ...
            if (plate_data->top_file.empty() || plate_data->pick_file.empty()) {
                ...
                if (plate_data->top_file.empty() || plate_data->pick_file.empty() || (!boost::filesystem::exists(plate_data->top_file)) ||
                    (!boost::filesystem::exists(plate_data->pick_file))) {
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                                                   "thumbnails stage: plate %1%'s top_file %2% also not there, need to regenerate") %
                                                   (i + 1) % plate_data->top_file;
                    if (!skip_this_plate) {
                    ...
                    if (regenerate_thumbnails) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s top_thumbnail file %2% cleared, need to regenerate") % (i + 1) %
                                   plate_data->top_file;
                        plate_data->top_file.clear();
                        plate_data->pick_file.clear();
                    } else
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s top_thumbnail file exists, no need to regenerate") % (i + 1);
                }
            }
        }
        ...
        if (need_regenerate_thumbnail || need_regenerate_no_light_thumbnail || need_regenerate_top_thumbnail) {
            ...
            for (const std::string& color : colors) {
                ...
                colors_out[color_idx] = ColorRGBA(float(rgb_color[0]) / 255.f, float(rgb_color[1]) / 255.f, float(rgb_color[2]) / 255.f,
                                                  float(rgb_color[3]) / 255.f);
            }
                        ...
                        for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                            ...
                            if (plate_data->plate_thumbnail.is_valid()) {
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("Line %1%: regenerate thumbnail, reset plate %2%'s thumbnail.") % __LINE__ %
                                               (i + 1);
                                    plate_data->plate_thumbnail.reset();
                                } else {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("plate %1% has a valid thumbnail, width %2%, height %3%， directly using it") %
                                               (i + 1) % plate_data->plate_thumbnail.width % plate_data->plate_thumbnail.height;
                                }
                            } else if (!plate_data->thumbnail_file.empty() && (boost::filesystem::exists(plate_data->thumbnail_file))) {
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format(
                                               "Line %1%: regenerate thumbnail, clear plate %2%'s thumbnail file path to empty.") %
                                               __LINE__ % (i + 1);
                                    plate_data->thumbnail_file.clear();
                                } else {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("plate %1% has a valid thumbnail %2% extracted from 3mf, directly using it") %
                                               (i + 1) % plate_data->thumbnail_file;
                                ...
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("Line %1%: regenerate thumbnail, Skip plate %2%.") % __LINE__ % (i + 1);
                                } else {
                                    ...
                                    case Slic3r::GUI::OpenGLManager::EFramebufferType::Arb: {
                                        ...
                                        Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer(*thumbnail_data, thumbnail_width,
                                                                                              thumbnail_height, thumbnail_params,
                                                                                              partplate_list, model.objects,
                                    ...
                                    case Slic3r::GUI::OpenGLManager::EFramebufferType::Ext: {
                                        ...
                                        Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer_ext(*thumbnail_data, thumbnail_width,
                                                                                                  thumbnail_height, thumbnail_params,
                                                                                                  partplate_list, model.objects,
                                                                                                  glvolume_collection, colors_out, shader,
                                                                                                  Slic3r::GUI::Camera::EType::Ortho);
                                    ...
                                    default: BOOST_LOG_TRIVIAL(info) << boost::format("framebuffer_type: unknown"); break;
                                    }
                            ...
                            // no light thumbnail
                            if (!plate_data->no_light_thumbnail_file.empty() &&
                                (boost::filesystem::exists(plate_data->no_light_thumbnail_file))) {
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format(
                                               "Line %1%: regenerate thumbnail, clear plate %2%'s no_light_thumbnail_file path to empty.") %
                                               __LINE__ % (i + 1);
                                    plate_data->no_light_thumbnail_file.clear();
                                } else
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format(
                                               "plate %1% has valid no_light_thumbnail_file extracted from 3mf, directly using it") %
                                               (i + 1);
                            } else {
                                ...
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("plate %1%'s no_light_thumbnail_file missed, need to regenerate") % (i + 1);
                                    switch (Slic3r::GUI::OpenGLManager::get_framebuffers_type()) {
                                    case Slic3r::GUI::OpenGLManager::EFramebufferType::Arb: {
                                        BOOST_LOG_TRIVIAL(info) << boost::format("framebuffer_type: ARB");
                                        Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer(*no_light_thumbnail, thumbnail_width,
                                                                                              thumbnail_height, thumbnail_params,
                                                                                              partplate_list, model.objects,
                                                                                              glvolume_collection, colors_out, shader,
                                                                                              Slic3r::GUI::Camera::EType::Ortho, false,
                                                                                              false, true);
                                        break;
                                    }
                                    case Slic3r::GUI::OpenGLManager::EFramebufferType::Ext: {
                                        BOOST_LOG_TRIVIAL(info) << boost::format("framebuffer_type: EXT");
                                        Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer_ext(*no_light_thumbnail, thumbnail_width,
                                                                                                  thumbnail_height, thumbnail_params,
                                                                                                  partplate_list, model.objects,
                                                                                                  glvolume_collection, colors_out, shader,
                                                                                                  Slic3r::GUI::Camera::EType::Ortho, false,
                            ...
                            /*if (part_plate->top_thumbnail_data.is_valid() && part_plate->pick_thumbnail_data.is_valid()) {
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info) << boost::format("Line %1%: regenerate thumbnail, reset plate %2%'s top/pick
                            thumbnail.")%__LINE__%(i+1); part_plate->top_thumbnail_data.reset(); part_plate->pick_thumbnail_data.reset();
                                    plate_data->top_file.clear();
                                    plate_data->pick_file.clear();
                                }
                                else {
                                    ...
                                    BOOST_LOG_TRIVIAL(info) << boost::format("plate %1% has a valid top/pick thumbnail data, directly using
                            it")%(i+1);
                                }
                            }
                            else*/
                            if ((!plate_data->top_file.empty() && (boost::filesystem::exists(plate_data->top_file))) &&
                                ...
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format(
                                               "Line %1%: regenerate thumbnail, clear plate %2%'s top/pick thumbnail file path to empty.") %
                                               __LINE__ % (i + 1);
                                    plate_data->top_file.clear();
                                    plate_data->pick_file.clear();
                                } else
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("plate %1% has valid top/pick thumbnail extracted from 3mf, directly using it") %
                                               (i + 1);
                            } else {
                                ...
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%'s top/pick thumbnail missed, need to regenerate, "
                                                                             "objects count %2%, skip_useless_pick %3%") %
                                                                   (i + 1) % plate_object_count[i] % skip_useless_pick;
                                    if (skip_useless_pick && ((plate_object_count[i] <= 1) || (plate_object_count[i] > 64))) {
                                        ...
                                        case Slic3r::GUI::OpenGLManager::EFramebufferType::Arb: {
                                            BOOST_LOG_TRIVIAL(info) << boost::format("framebuffer_type: ARB");
                                            Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer(*top_thumbnail, thumbnail_width,
                                                                                                  thumbnail_height, thumbnail_params,
                                                                                                  partplate_list, model.objects,
                                                                                                  glvolume_collection, colors_out, shader,
                                                                                                  Slic3r::GUI::Camera::EType::Ortho, true,
                                                                                                  false);
                                            Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer(*picking_thumbnail, thumbnail_width,
                                                                                                  thumbnail_height, thumbnail_params,
                                                                                                  partplate_list, model.objects,
                                                                                                  glvolume_collection, colors_out, shader,
                                                                                                  Slic3r::GUI::Camera::EType::Ortho, true,
                                        ...
                                        case Slic3r::GUI::OpenGLManager::EFramebufferType::Ext: {
                                            BOOST_LOG_TRIVIAL(info) << boost::format("framebuffer_type: EXT");
                                            Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer_ext(*top_thumbnail, thumbnail_width,
                                                                                                      thumbnail_height, thumbnail_params,
                                                                                                      partplate_list, model.objects,
                                                                                                      glvolume_collection, colors_out,
                                                                                                      shader,
                                                                                                      Slic3r::GUI::Camera::EType::Ortho,
                                                                                                      true, false);
                                            Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer_ext(*picking_thumbnail, thumbnail_width,
                                                                                                      thumbnail_height, thumbnail_params,
                                                                                                      partplate_list, model.objects,
                                                                                                      glvolume_collection, colors_out,
                                                                                                      shader,
                            ...
                            if (need_create_top_group) {
                                ...
                                BOOST_LOG_TRIVIAL(info)
                                    << boost::format("plate %1%: add thumbnail data for top and pick into group") % (i + 1);
                            }
                        }
                    }
                }
            ...
            for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                ...
                if (skip_this_plate) {
                    ...
                    plate_data->top_file.clear();
                    plate_data->pick_file.clear();
                } else if (!plate_data->plate_thumbnail.is_valid() && !plate_data->thumbnail_file.empty() &&
                           (boost::filesystem::exists(plate_data->thumbnail_file))) {
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format("no need to generate: plate %1% has a valid thumbnail %2% extracted from 3mf, convert to data") %
                               (i + 1) % plate_data->thumbnail_file;
                    int dec_ret = decode_png_to_thumbnail(plate_data->thumbnail_file, plate_data->plate_thumbnail);
                    if (!dec_ret) {
                        BOOST_LOG_TRIVIAL(info) << boost::format("decode png to mem sucess.");
        ...
        for (int i = 0; i < partplate_list.get_plate_count(); i++) {
            ...
            if (plate_bed_type == btDefault) {
                ...
                if (cur_bed_type) {
                    BOOST_LOG_TRIVIAL(info) << boost::format("plate %1% bed type: %2%, set from global") % (i + 1) %
                                                   cur_bed_type->serialize();
        ...
        if (!metadata_name.empty()) {
            ...
            for (unsigned int i = 0; i < metadata_name.size(); i++) {
                BOOST_LOG_TRIVIAL(info) << boost::format("index %1% metadata_name %2%, metadata_value %3%") % i % metadata_name[i] %
                                               metadata_value[i];
            }
        }

        if (!this->export_project(&m_models[0], export_3mf_file, plate_data_list, project_presets, thumbnails, no_light_thumbnails,
                                  top_thumbnails, pick_thumbnails, calibration_thumbnails, plate_bboxes, &m_print_config, minimum_save,
                                  plate_to_slice - 1)) {
...
{
    boost::nowide::cout << SLIC3R_APP_KEY << "-" << SLIC3R_VERSION << ":" << std::endl
...
bool CLI::export_project(Model*                       model,
                         std::string&                 path,
...
std::string CLI::output_filepath(const ModelObject& object, unsigned int index, IO::ExportFormat format, std::string path_dir) const
{
    ...
    BOOST_LOG_TRIVIAL(trace) << __FUNCTION__ << ": dir = " << path_dir << ", file_name=" << file_name << ", pos = " << pos
                             << ", ext_pos=" << ext_pos;
    if (path_dir.empty()) {
        std::string cmdline_param = m_config.opt_string("outputdir");
        if (!cmdline_param.empty()) {
...
std::string TraceStack()
{
    ...
    for (WORD i = 0; i < frames; ++i) {
        ...
        if (SymFromAddr(process, address, &displacementSym, pSymbol)
            ...
            oss << "\t" << pSymbol->Name << " at " << line.FileName << ":" << line.LineNumber << "(0x" << std::hex << pSymbol->Address <<
std::dec << ")" << std::endl;
        }
        else {
