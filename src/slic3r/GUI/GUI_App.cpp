...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #2573D9  -- 73
        if (bg_col != orig_col) {
    ...
    handler->Bind(wxEVT_WEBREQUEST_COMPLETED, [by_user, compareVersions, this](wxWebRequestEvent& event) {
        if (event.GetState() == wxWebRequest::State_Completed) {
            ...
            try {
                ...
                std::string changelog_utf8 = windows["changelog"].get<std::string>();
                wxString changelog = wxString::FromUTF8(changelog_utf8.c_str());
    ...
    std::thread([this, download_url, remote_version, sha256, progress_ptr, is_cancelled, update_status]() {
        ...
        try {
            ...
            GUI::wxGetApp().CallAfter([progress_ptr, remote_version]() {
                wxMessageBox(
                    wxString::Format(_L("HIM profiles have been updated to %s.\n\nPlease restart the application to apply changes."),
                                     remote_version),
