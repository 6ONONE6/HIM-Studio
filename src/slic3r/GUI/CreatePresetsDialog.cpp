...
static const std::vector<std::string> printer_vendors = {"Anker",      "Anycubic",   "Artillery", "Bambulab",
                                                         "BIQU",       "Comgrow",    "Creality",  "Custom Printer",
                                                         "Elegoo",     "Flashforge", "FLSun",     "FlyingBear",
                                                         "Folgertech", "InfiMech",   "Kingroon",  "Orca Arena Printer",
                                                         "Peopoly",    "Prusa",      "Qidi",      "Raise3D",
                                                         "RatRig",     "SecKit",     "Snapmaker", "Sovol",
                                                         "Tronxy",     "TwoTrees",   "UltiMaker", "Vivedino",
                                                         "Voron",      "Voxelab",    "Vzbot",     "Wanhao"};
...
static const std::unordered_map<std::string, std::vector<std::string>> printer_model_map =
    {{"Anker", {"Anker M5", "Anker M5 All-Metal Hot End", "Anker M5C"}},
     ...
     {"BIQU", {"BX"}},
     {"Creality ENDER",
      {"Ender-3", "Ender-3 BLTouch", "Ender-3 Pro", "Ender-3 Neo", "Ender-3 V2 Neo", "Ender-3 S1 Plus", "Ender-3 Max", "Ender-3 Max Neo",
       "Ender-4", "Ender-5 Pro", "Ender-5 Pro", "Ender-7", "Ender-2", "Ender-2 Pro"}},
     {"Creality CR",
      {"CR-5 Pro", "CR-5 Pro H", "CR-10 SMART", "CR-10 SMART Pro", "CR-10 Mini", "CR-10", "CR-10 v3", "CR-10 S", "CR-10 v2", "CR-10 v2",
       "CR-10 S Pro", "CR-10 S Pro v2", "CR-10 S4", "CR-10 S5", "CR-20", "CR-20 Pro", "CR-200B", "CR-8"}},
     ...
     {"Geeetech", {"Thunder", "Thunder Pro", "Mizar s",    "Mizar Pro",   "Mizar",        "Mizar Max", "Mizar M", "A10 Pro",
                   "A10 M",   "A10 T",       "A20",        "A20 M",       "A20T",         "A30 Pro",   "A30 M",   "A30 T",
                   "E180",    "Me Ducer",    "Me creator", "Me Creator2", "GiantArmD200", "l3 ProB",   "l3 Prow", "l3 ProC"}},
     ...
     {"MakerGear",
      {"Micro", "M2(V4 Hotend)", "M2 Dual", "M3-single Extruder", "M3-Independent Dual Rev.0", "M3-Independent Dual Rev.0(Duplication Mode)",
     ...
     {"TriLAB",
      {"AzteQ Industrial", "AzteQ Dynamic", "DeltiQ 2", "DeltiQ 2 Plus", "DeltiQ 2 + FlexPrint 2", "DeltiQ 2 Plus + FlexPrint 2",
       "DeltiQ 2 +FlexPrint", "DeltiQ 2 Plus + FlexPrint", "DeltiQ M", "DeltiQ L", "DeltiQ XL"}},
     {"Trimaker", {"Nebula cloud", "Nebula", "Cosmos ll"}},
     {"Ultimaker", {"Ultimaker 2"}},
     {"Voron", {"v2 250mm3", "v2 300mm3", "v2 350mm3", "v1 250mm3", "v1 300mm3", "v1 350mm3", "Zero 120mm3", "Switchwire"}},
...
wxBoxSizer* CreateFilamentPresetDialog::create_button_item()
{
    ...
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(27, 85, 161), StateColor::Pressed),
    ...
    m_button_create->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent& e) {
        ...
        if (!m_can_not_find_vendor_checkbox->GetValue()) {
            ...
            if (m_filament_custom_vendor_input->GetTextCtrl()->GetValue().empty()) {
                ...
                if (vendor_name == "Bambu" || vendor_name == "Generic") {
                    MessageDialog dlg(this, _L("\"Bambu\" or \"Generic\" can not be used as a Vendor for custom filaments."),
                                      wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES | wxYES_DEFAULT | wxCENTRE);
                    dlg.ShowModal();
                    return;
                }
        ...
        if (m_can_not_find_vendor_checkbox->GetValue() && str_is_all_digit(vendor_name)) {
            MessageDialog dlg(this, _L("The vendor can not be a number. Please re-enter."),
                              wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES | wxYES_DEFAULT | wxCENTRE);
            dlg.ShowModal();
            return;
        }
        ...
        if (preset_bundle->filaments.is_alias_exist(filament_preset_name)) {
            MessageDialog dlg(this,
                              wxString::Format(_L("The Filament name %s you created already exists. \nIf you continue creating, the preset "
                                                  "created will be displayed with its "
        ...
        if (i == btn_idx) {
            ...
            if (curr_selected_type == m_create_type.base_filament) {
                ...
                m_filament_preset_text->SetLabel(_L("We would rename the presets as \"Vendor Type Serial @printer you selected\". \nTo add "
                                                    "preset for more printers, Please go to printer selection"));
...
wxBoxSizer* CreatePrinterPresetDialog::create_printer_item(wxWindow* parent)
{
    ...
    m_select_vendor->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent e) {
        ...
        if (iter != printer_model_map.end()) {
            ...
            MessageDialog dlg(this, _L("The model is not found, please reselect vendor."),
                              wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES | wxYES_DEFAULT | wxCENTRE);
            dlg.ShowModal();
        }
        e.Skip();
...
bool CreatePrinterPresetDialog::load_system_and_user_presets_with_curr_model(PresetBundle& temp_preset_bundle, bool just_template)
{
    ...
    if (PRESET_CUSTOM_VENDOR == m_printer_preset_vendor_selected.name || PRESET_CUSTOM_VENDOR == m_printer_preset_vendor_selected.id) {
        ...
        if (preset_path.empty()) {
            ...
            MessageDialog dlg(this, _L("Preset path is not find, please reselect vendor."),
                              wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES_NO | wxYES_DEFAULT | wxCENTRE);
            dlg.ShowModal();
            return false;
        }
    ...
    if (index_at != std::string::npos && index_nozzle != std::string::npos) {
        ...
        MessageDialog dlg(this, _L("The nozzle diameter is not found, please reselect."),
                          wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES_NO | wxYES_DEFAULT | wxCENTRE);
        dlg.ShowModal();
        return false;
    }
    ...
    if (temp_printer_preset) {
        ...
        MessageDialog dlg(this, _L("The printer preset is not found, please reselect."),
                          wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES_NO | wxYES_DEFAULT | wxCENTRE);
        dlg.ShowModal();
        return false;
    }
        ...
        if (curr_selected_printer_type == m_create_type.create_printer) {
            if (m_can_not_find_vendor_combox->GetValue()) {
                ...
                if (custom_vendor.empty() || custom_model.empty()) {
                    MessageDialog dlg(this, _L("The custom printer or model is not entered, please enter it."),
                                      wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES | wxYES_DEFAULT | wxCENTRE);
                    dlg.ShowModal();
                    show_page1();
                    return;
...
bool CreatePrinterPresetDialog::data_init()
{
    ...
    m_printer_vendor->Bind(wxEVT_COMBOBOX, [this, vendors, nozzle](wxCommandEvent e) {
        ...
        if (iterator != vendors.end()) {
            ...
            MessageDialog dlg(this, _L("Vendor is not find, please reselect."), wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"),
                              wxYES_NO | wxYES_DEFAULT | wxCENTRE);
            dlg.ShowModal();
            return;
        }
        ...
        if (printer_preset_model.size() == 0) {
            MessageDialog dlg(this, _L("Current vendor has no models, please reselect."),
                              wxString(SLIC3R_APP_FULL_NAME) + " - " + _L("Info"), wxYES | wxYES_DEFAULT | wxCENTRE);
            dlg.ShowModal();
            return;
        }
...
void CreatePrinterPresetDialog::set_current_visible_printer()
{
    ...
    for (const Preset& printer_preset : printer_presets) {
        if (printer_preset.is_system || !printer_preset.is_visible)
            continue;
...
CreatePresetSuccessfulDialog::CreatePresetSuccessfulDialog(wxWindow* parent, const SuccessType& create_success_type)
    : DPIDialog(parent ? parent : nullptr,
                wxID_ANY,
                PRINTER == create_success_type ? _L("Create Printer Successful") : _L("Create Filament Successful"),
                wxDefaultPosition,
                wxDefaultSize,
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    case FILAMENT:
        ...
        wxString sync_text   = sync_user_preset_need_enabled ?
                                   _L("\n\nOrca has detected that your user presets synchronization function is not enabled, which may "
                                        "result in unsuccessful Filament settings on "
    ...
    switch (create_success_type) {
    case PRINTER: m_button_ok = new Button(this, _L("Printer Setting")); break;
    case FILAMENT:
        m_button_ok = sync_user_preset_need_enabled ? new Button(this, _L("Sync user presets")) : new Button(this, _L("OK"));
        break;
    }
    StateColor btn_bg_green(std::pair<wxColour, int>(wxColour(27, 85, 161), StateColor::Pressed),
                            std::pair<wxColour, int>(wxColour(43, 135, 255), StateColor::Hovered),
...
wxBoxSizer* ExportConfigsDialog::create_export_config_item(wxWindow* parent)
{
    ...
    wxStaticText* static_export_printer_preset_bundle_text =
        new wxStaticText(parent, wxID_ANY,
                         _L("Printer and all the filament&&process presets that belongs to the printer. \nCan be shared with others."),
                         wxDefaultPosition, wxDefaultSize);
...
wxPanel* PresetTree::get_child_item(wxPanel* parent, std::shared_ptr<Preset> preset, std::string printer_name, int preset_index, bool is_last)
{
    ...
    if (base_id_error) {
        ...
        wxHyperlinkCtrl* m_download_hyperlink = new wxHyperlinkCtrl(panel, wxID_ANY, _L("[Delete Required]"), wiki_url, wxDefaultPosition,
