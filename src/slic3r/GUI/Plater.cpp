...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    // update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - HIM-Studio");
...
void Plater::_calib_pa_pattern(const Calib_Params& params)
{
    ...
    if (accels.empty()) {
        ...
        const auto msg{_L("INFO:") + "\n" + _L("No accelerations provided for calibration. Use default acceleration value ") +
                       std::to_string(long(accel)) + _L("mm/s²")};
    ...
    if (speeds.empty()) {
        double speed = CalibPressureAdvance::find_optimal_PA_speed(wxGetApp().preset_bundle->full_config(),
                                                                   (fabs(print_config.get_abs_value("line_width", nozzle_diameter)) <=
                                                                    DBL_EPSILON) ?
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    auto dont_show_again = create_remember_checkbox(_L("Remember my choice."), this,
                                                    _L("This option can be changed later in preferences, under 'Load Behaviour'."));
    ...
    m_confirm->SetBorderColor(wxColour(37, 115, 217));
...
void Plater::clone_selection()
{
    if (is_selection_empty())
        return;
    long     res = wxGetNumberFromUser("", _L("Clone"), _L("Number of copies:"), 1, 0, 1000, this);
    wxString msg;
    if (res == -1) {
