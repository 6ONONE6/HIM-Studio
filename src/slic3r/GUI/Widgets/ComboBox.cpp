    ...
    : drop(texts, tips, icons)
{
    ...
    if (style & wxCB_READONLY) {
        ...
        TextInput::SetBorderColor(StateColor(std::make_pair(0xDBDBDB, (int) StateColor::Disabled),
                                             std::make_pair(0x2573D9, (int) StateColor::Hovered),
                                             std::make_pair(0xDBDBDB, (int) StateColor::Normal)));
        TextInput::SetBackgroundColor(
            StateColor(std::make_pair(0xF0F0F1, (int) StateColor::Disabled),
                       std::make_pair(0xDAEEFF, (int) StateColor::Focused), // ORCA updated background color for focused item E5F0EE
...
void ComboBox::SetSelection(int n)
{
    ...
    if (drop.selection >= 0 && drop.iconSize.y > 0)
        SetIcon(icons[drop.selection].IsNull() ? create_scaled_bitmap("drop_down", nullptr, 16) :
                                                 icons[drop.selection]); // ORCA fix combo boxes without arrows
}
...
int ComboBox::Append(const wxString& item, const wxBitmap& bitmap) { return Append(item, bitmap, nullptr); }

int ComboBox::Append(const wxString& item, const wxBitmap& bitmap, void* clientData)
{
...
wxString ComboBox::GetString(unsigned int n) const { return n < texts.size() ? texts[n] : wxString{}; }
