...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{0x2573D9, (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})
{
...
void SwitchButton::SetTextColor(StateColor const& color) { text_color = color; }

void SwitchButton::SetTextColor2(StateColor const& color) { text_color2 = color; }

void SwitchButton::SetTrackColor(StateColor const& color) { track_color = color; }

void SwitchButton::SetThumbColor(StateColor const& color) { thumb_color = color; }
...
void SwitchButton::Rescale()
{
    if (labels[0].IsEmpty()) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            memdc.DrawText(labels[1],
                           {trackSize.x - thumbSize.x - BS + (thumbSize.x - textSize[1].x) / 2, BS + (thumbSize.y - textSize[1].y) / 2});
...
void SwitchButton::update() { SetBitmap((GetValue() ? m_on : m_off).bmp()); }
