...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(37, 115, 217));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(37, 115, 217)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(27, 85, 161), StateColor::Pressed), std::pair<wxColour, int>(wxColour(43, 135, 255), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(37, 115, 217), StateColor::Normal));
