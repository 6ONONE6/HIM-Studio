...
#  updated by suKing on 15/5/26.

list_file="./localization/i18n/list.txt"
pot_file="./localization/i18n/HIMstudio_.pot"
filtered_list=""
missing_list=""
generated_root_dir=""
...
if $FULL_MODE; then
    ...
    if $has_sources; then
        ...
        generated_pot_file="${generated_i18n_dir}/HIMstudio_.pot"
...
do
    ...
    if [ -f "$dir/HIMstudio_${lang}.po" ]; then
        if $FULL_MODE && [ -f "$pot_file" ]; then
            merged_po=$(mktemp)
            if ! msgmerge -N -o "$merged_po" "$dir/HIMstudio_${lang}.po" "$pot_file"; then
